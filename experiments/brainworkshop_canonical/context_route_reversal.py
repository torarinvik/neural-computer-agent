"""Audit nonstationary replacement of one cue's opaque route.

The same rendered cue first trains n-back-2 in slot zero, then a new n-back-4
capability is acquired without replaying the old task. A fresh candidate
calibration transaction changes the cue's preferred route while a forced audit
checks that the old capability remains intact.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.old_n_back == args.new_n_back:
        raise ValueError("reversal needs different old and new n-back families")
    ...
    cue_shuffle_controls = _controls(
        agent,
        n_back=args.new_n_back,
        batch_size=args.batch_size,
        steps=args.steps,
    ...
    cue_event = agent.runtime.encoders["stimulus"](
        torch.tensor([REVERSAL_CUE], dtype=torch.long)
    )[0]
    ...
    old_retention_score = min(
        row["eligible_accuracy"] for row in old_retention
    )
    ...
    report = {
        "schema": "neural-computer.brainworkshop-context-route-reversal.v1",
        "status": (
            "promoted_nonstationary_context_reversal"
            if route_order[0] == new_slot
            and new_route_passed
            ...
            else "unpromoted_context_reversal"
        ),
        "cue_symbol": REVERSAL_CUE,
        "old_n_back": args.old_n_back,
        "new_n_back": args.new_n_back,
        "old_controls_before": old_controls_before,
        "new_controls_after": new_controls,
        "cue_shuffle_controls": cue_shuffle_controls,
        "old_calibration": old_calibration,
        "new_calibration": new_calibration,
        "old_retention_after_reversal": old_retention,
        "preferred_route_order_after_reversal": route_order,
        ...
        * (args.old_updates + args.new_updates),
        ...
        "promotion_gates": {
            "new_route": new_route_passed,
            "new_calibration": new_calibration_score >= 0.8,
            "old_capability_retained": old_retention_score >= 0.8,
            "route_state_reload": reload_route == [new_slot, new_slot],
            "prior_bank_unchanged": old_bank_digest == new_bank_digest,
        },
        "claim_boundary": (
            "A same-cue replacement can commit a new opaque route while the "
            "old capability remains externally retained; this is bounded "
            "nonstationary route replacement, not general continual learning."
        ),
    }
