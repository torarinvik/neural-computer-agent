"""Replay a saved grounded-caption frontend through the frozen runtime."""
...
def _runtime_from_saved_caption(
    ...
    if artifact.get("schema") != "amodal-caption-aligned-frontend-v1":
        raise ValueError("unsupported caption frontend artifact schema")
    if artifact.get("controller_sha256") != _sha256(controller_path):
        raise ValueError("caption frontend targets another controller")
    if artifact.get("input_bus_sha256") != _sha256(bus_path):
        raise ValueError("caption frontend targets another input bus")
    ...
    captions = PairRelationCaptionEncoder(
        extracted.controller.width,
        vocabulary=int(artifact["caption_vocabulary"]),
        embedding_width=int(artifact["embedding_width"]),
    ).to(device)
    captions.load_state_dict(
        {name: value.to(device=device) for name, value in artifact["state_dict"].items()}
    )
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=999901)
    ...
    runtime = _runtime_from_saved_caption(
    ...
    report = {
        "schema": "amodal-caption-frontend-replay-v1",
