...
> **Repository boundary:** this directory is now historical experiment and
> audit machinery. The canonical runtime is `src/neural_computer/`; new agent
> code must import `neural_computer`. The legacy-shaped controller and
> checkpoint converters remain here only to replay prior evidence while those
> experiments are migrated.
...
```sh
python -m experiments.archive.unified_cognitive_controller.convert_amodal_checkpoint \
...
```sh
python -m \
  experiments.archive.unified_cognitive_controller.canonicalize_intention_checkpoint \
  artifacts/checkpoints/unified_repertoire_span2_strict_seed122005.pt \
...
```bash
python -m experiments.archive.unified_cognitive_controller.train \
  --device cuda \
  ...
  --trials 6 \
  --feedback-trials 1 \
  --report experiments/archive/unified_cognitive_controller/reports/combined_atoms.json \
...
python -m experiments.archive.unified_cognitive_controller.train \
  ...
  --batch-size 256 \
  --trials 6 \
  --report experiments/archive/unified_cognitive_controller/reports/hidden_rule_smoke.json \
...
```bash
python -m experiments.archive.unified_cognitive_controller.audit_checkpoint \
  --device cuda \
  --task constant_action \
  --checkpoint artifacts/checkpoints/unified_constant_action_v1_seed2501.pt \
  --seed 2707 \
  --report experiments/archive/unified_cognitive_controller/reports/constant_action_blind_seed2707.json
```
