...
def _outcome_only_position_weights(
        outcomes: torch.Tensor, replay_mask: torch.Tensor, *,
        target_lifetimes: int, span: int, power: float,
        floor: float) -> torch.Tensor:
    """Prioritize hard within-lifetime positions from scalar outcomes only."""
    if target_lifetimes < 1 or span < 1:
        raise ValueError("target lifetimes and span must be positive")
    if power < 0.0:
        raise ValueError("query position difficulty power must not be negative")
    if not 0.0 <= floor <= 1.0:
        raise ValueError("query position difficulty floor must be within [0, 1]")
    if outcomes.ndim != 1 or replay_mask.ndim != 1:
        raise ValueError("outcomes and replay mask must be one-dimensional")
    if outcomes.shape != replay_mask.shape:
        raise ValueError("outcomes and replay mask must have equal length")
    weights = torch.ones_like(outcomes)
    if power == 0.0:
        return weights
    target_rows = target_lifetimes * span
    if target_rows > outcomes.shape[0]:
        raise ValueError("target stream is shorter than its declared size")
    target = torch.zeros_like(replay_mask)
    target[:target_rows] = True
    target &= ~replay_mask
    if not bool(target.any()):
        return weights
    for position in range(span):
        start = position * target_lifetimes
        end = start + target_lifetimes
        bucket = target[start:end]
        if not bool(bucket.any()):
            continue
        difficulty = (1.0 - outcomes[start:end][bucket].mean()).clamp_min(floor)
        weights[start:end][bucket] = difficulty.pow(power)
    mean = weights[target].mean().clamp_min(torch.finfo(weights.dtype).eps)
    weights[target] = weights[target] / mean
    return weights
...
def _collect_buffer(
        ...
        operation: str = "mixed", blank_sequence: bool = False,
        ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    batch = generate_sequence_memory_batch(
        ...
        blank_sequence=blank_sequence, device=device)
    null = torch.full(
...
def main() -> None:
    ...
    parser.add_argument(
        "--missing-evidence-lifetimes", type=int, default=0,
        help=(
            "collect blank-sequence scalar-outcome rehearsal; requires "
            "--protect-rehearsal and is charged as unique experience"))
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "difficulty power"))
    parser.add_argument(
        "--query-position-difficulty-power", type=float, default=0.0,
        help=(
            "outcome-only curriculum: upweight hard fresh target query "
            "positions using their generic within-stream coordinate"))
    parser.add_argument(
        "--query-position-difficulty-floor", type=float, default=0.25,
        help=(
            "minimum position difficulty before applying the query-position "
            "difficulty power"))
    parser.add_argument(
    ...
    if (
            ...
            or args.missing_evidence_lifetimes < 0
            or args.rehearsal_lifetimes % 2
            or args.span < 1
    ...
    if args.missing_evidence_lifetimes % 2:
        raise ValueError("missing-evidence lifetimes must be even")
    if args.missing_evidence_lifetimes and not args.protect_rehearsal:
        raise ValueError(
            "missing-evidence rehearsal requires --protect-rehearsal")
    ...
    if not 0.0 <= args.query_difficulty_floor <= 1.0:
        raise ValueError("query-difficulty-floor must be within [0, 1]")
    if args.query_position_difficulty_power < 0:
        raise ValueError(
            "query-position-difficulty-power must not be negative")
    if not 0.0 <= args.query_position_difficulty_floor <= 1.0:
        raise ValueError(
            "query-position-difficulty-floor must be within [0, 1]")
    ...
    if args.buffer_in is not None:
        ...
        if args.missing_evidence_lifetimes:
            buffer_parts.append(_collect_buffer(
                base, count=args.missing_evidence_lifetimes, span=args.span,
                distractors=args.distractors,
                seed=data_seed + 3_000_007,
                device=device,
                position_augmentation=args.position_augmentation,
                include_intention=include_intention,
                include_workspace=include_workspace,
                ...
                blank_sequence=True))
            stream_specs.append({
                "kind": "missing_evidence", "span": args.span,
                "lifetimes": args.missing_evidence_lifetimes})
    ...
    rehearsal_lifetime_count = (
        ...
        + args.missing_evidence_lifetimes
    ...
    query_sample_weights = query_sample_weights * (
        _outcome_only_position_weights(
            outcomes, protected_mask,
            target_lifetimes=args.train_lifetimes,
            span=args.span,
            power=args.query_position_difficulty_power,
            floor=args.query_position_difficulty_floor))
    ...
    report = {
        ...
        "missing_evidence_lifetimes": args.missing_evidence_lifetimes,
        ...
        "query_position_difficulty_power": (
            args.query_position_difficulty_power),
        "query_position_difficulty_floor": (
            args.query_position_difficulty_floor),
