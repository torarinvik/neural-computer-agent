"""Replay an independently saved token frontend through the frozen runtime."""
...
def _runtime_from_saved_token(
    ...
    if artifact.get("schema") != "amodal-token-aligned-frontend-v1":
        raise ValueError("unsupported token frontend artifact schema")
    if artifact.get("controller_sha256") != _sha256(controller_path):
        raise ValueError("token frontend was trained against another controller")
    if artifact.get("input_bus_sha256") != _sha256(bus_path):
        raise ValueError("token frontend was trained against another input bus")
    ...
    token_encoder = PairRelationTokenEncoder(
        extracted.controller.width,
        grid=int(artifact["token_grid"]),
        levels=int(artifact["token_levels"]),
        embedding_width=int(artifact["embedding_width"]),
    ).to(device)
    token_encoder.load_state_dict(
        {
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=996901)
    ...
    runtime = _runtime_from_saved_token(
    ...
    report = {
        "schema": "amodal-token-frontend-replay-v1",
