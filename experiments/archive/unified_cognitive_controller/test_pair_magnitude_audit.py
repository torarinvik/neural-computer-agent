...
def test_pair_magnitude_audit_counterfactual_is_pixel_valid() -> None:
    normal = generate_lifetimes(
        64, 6, seed=22401, heldout=True,
        task="visible_pair_magnitude")
    reversed_order = generate_lifetimes(
        64, 6, seed=22401, heldout=True,
        task="visible_pair_magnitude", reverse_contexts=True)
