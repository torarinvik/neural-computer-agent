...
OUT=experiments/forward_transfer_attention/targeted_mixed_confirmation
...
for seed in 11 23 37 51; do
  OMP_NUM_THREADS=10 MKL_NUM_THREADS=10 /venv/main/bin/python \
    ...
    --controller-seed "$seed" --consolidator-seed 23 --eval-lifetimes 1024 \
  ...
  for primitive in shape temporal; do
    OMP_NUM_THREADS=10 MKL_NUM_THREADS=10 /venv/main/bin/python \
      ...
      --eval-lifetimes 1024 --batch-size 64 --query-count 4 --seed "$seed" \
