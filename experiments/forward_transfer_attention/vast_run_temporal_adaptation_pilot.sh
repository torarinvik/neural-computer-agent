...
OUT=experiments/forward_transfer_attention/targeted_temporal_adaptation_pilot
mkdir -p "$OUT"
OMP_NUM_THREADS=64 MKL_NUM_THREADS=64 /venv/main/bin/python \
  ...
  --train-lifetimes 256 --eval-lifetimes 256 --batch-size 64 --epochs 2 \
  --query-count 4 --learning-rate 1e-4 --primitive temporal --seed 23 --device cuda \
  >"$OUT/seed_23.log" 2>&1
