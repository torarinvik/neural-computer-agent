...
OUT=experiments/forward_transfer_attention/targeted_early_transfer
mkdir -p "$OUT"
pids=()
for seed in 11 23 37 51; do
  OMP_NUM_THREADS=32 MKL_NUM_THREADS=32 /venv/main/bin/python \
    ...
    --initial-checkpoint "experiments/forward_transfer_attention/targeted_replication/seed_${seed}.pt" \
    --checkpoint "$OUT/seed_${seed}.pt" --report "$OUT/seed_${seed}.json" \
    --train-lifetimes 1024 --eval-lifetimes 512 --batch-size 64 \
    --epochs 6 --primitive-epochs 0 --read-top-k 1 --query-count 4 \
