...
OUT=experiments/forward_transfer_attention/targeted_temporal_atom_causal_audit
rm -rf "$OUT"
mkdir -p "$OUT"
pids=()
for condition in intact empty shuffled garbage; do
  OMP_NUM_THREADS=24 MKL_NUM_THREADS=24 /venv/main/bin/python \
    ...
    --controller-checkpoint experiments/forward_transfer_attention/targeted_joint_temporal_pilot/seed_23.pt \
