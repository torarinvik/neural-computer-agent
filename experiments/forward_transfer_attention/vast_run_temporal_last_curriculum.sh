...
OUT=experiments/forward_transfer_attention/targeted_temporal_last_curriculum
rm -rf "$OUT"
mkdir -p "$OUT"
OMP_NUM_THREADS=64 MKL_NUM_THREADS=64 /venv/main/bin/python \
  ...
  --train-lifetimes 256 --eval-lifetimes 256 --batch-size 64 --epochs 8 \
  --last-epochs 8 --evaluation-temporal-stage last --query-count 4 \
  --controller-learning-rate 5e-5 --consolidator-learning-rate 1e-4 \
  --seed 23 --device cuda >"$OUT/seed_23.log" 2>&1
