...
OUT=experiments/forward_transfer_attention/targeted_temporal_grounding_curriculum
rm -rf "$OUT"
mkdir -p "$OUT"
OMP_NUM_THREADS=64 MKL_NUM_THREADS=64 /venv/main/bin/python \
  ...
  --train-lifetimes 256 --eval-lifetimes 256 --batch-size 64 --epochs 10 \
  --grounding-epochs 5 --query-count 4 --controller-learning-rate 5e-5 \
