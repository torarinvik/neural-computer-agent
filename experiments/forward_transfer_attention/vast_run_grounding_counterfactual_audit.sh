...
OUT=experiments/forward_transfer_attention/targeted_grounding_counterfactual_audit
rm -rf "$OUT"
mkdir -p "$OUT"
pids=()
for mode in intact reversed counterfactual; do
  ...
  OMP_NUM_THREADS=32 MKL_NUM_THREADS=32 /venv/main/bin/python \
    ...
    --controller-checkpoint experiments/forward_transfer_attention/targeted_temporal_grounding_curriculum/seed_23.pt \
    --report "$OUT/${mode}.json" --eval-lifetimes 256 --batch-size 64 \
    --query-count 4 --seed 23 --primitive temporal --condition intact --device cuda \
