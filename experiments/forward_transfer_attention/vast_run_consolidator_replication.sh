...
OUT=experiments/forward_transfer_attention/targeted_consolidator_replication
mkdir -p "$OUT"
pids=()
for seed in 11 23 37 51; do
  OMP_NUM_THREADS=32 MKL_NUM_THREADS=32 /venv/main/bin/python \
    ...
    --train-lifetimes 1024 --eval-lifetimes 1024 --batch-size 64 \
    --epochs 12 --query-count 4 --learning-rate 3e-4 --seed "$seed" --device cuda \
