"""PLAN DEPTH: is the planner, not the interface, the binding constraint?

F225-F227 eliminated every interface-side account of the width null:
density (F226 v1), urgency (F226 v2), term shape (F226 v3/v4) and now
distinct ROLES (F227 pursuit worlds, where the six-slot flee-nearest
arm ties the eight-slot looming arm at t=-0.15). The surviving
explanation is the PLANNER: depth-1 greedy can only spend "where is
the nearest thing now", so every richer percept is dead weight. Role
information -- the bouncer will bounce away, the pursuer will not --
is only cashable by lookahead.

This probe measures exactly that, with everything else frozen. Goals
are selected at depth 1 by the F226 robust protocol (unchanged), then
the SAME goal, bank and encoder are executed on the evaluation stream
at depth 1 and at depth 2, where depth 2 composes the bank's one-step
programs over action pairs: cost(a) = c(s1) + min_b c(s2), s1 =
prog_a(ref), s2 = prog_b(s1). Selection is shared, so the contrast is
pure execution depth; goals tuned under depth-1 bias AGAINST depth 2,
which is the conservative direction.

Registered predictions:
  1. Depth 2 improves the pursue worlds (anticipating a closing
     pursuer) and intercept2 (getting under a faller two steps out);
     pooled paired t >= +2 there.
  2. Genuine risk, recorded either way: composing one-step programs
     SQUARES model error (hazard bounces and respawns are not in the
     programs), so depth 2 may lose. A null/negative verdict means
     depth is not the binding constraint at this horizon and the
     planner needs a better model, not a deeper search.
  3. avoid2 anchor: small or no change (flee-nearest is near ceiling).
"""
...
for name, config in WORLDS:
    ...
    for label, candidate in ENC_CANDIDATES.items():
        globals()["enc"] = candidate
        goal = choose_composite(config, banks[label], frozenset(range(SLOTS)))
        if goal is None:
            continue
    ...
    row = {"encoder": label,
           ...
           "depth1": play(config, "bank", bank, args.seed * 977, goal,
                          plant_executor, args.episodes, args.steps),
           "depth2": play(config, "bank", bank, args.seed * 977, goal,
                          plant_executor, args.episodes, args.steps,
                          depth=2),
           "oracle": play(config, "oracle", None, args.seed * 977, goal,
                          plant_executor, args.episodes, args.steps)}
    results[name] = row
    print(f"  {name:<16} random {row['random']:+.3f}  d1 "
          f"{row['depth1']:+.3f}  d2 {row['depth2']:+.3f}  "
          f"[{label}]", flush=True)
