"""RING TRANSFER: the third substrate -- amodal value knowledge as
a pattern, not a coincidence (F270).

F267 proved grid-learned class bindings run the graph substrate at
native ceiling. Two substrates are a line; three are a pattern.
This probe adds a RING world -- 12 cells, cyclic metric
d(i,j) = min(|i-j|, 12-|i-j|), actions {left, right}, food (+1,
respawns), hazard walker (-1, fatal on co-location) -- and applies
the SAME grid-fit five-coefficient class binding zero-shot, contact
kernel calibrated to the ring's step metric ((2-d)/2, diameter 6).

Arms per ring world (rcollect1, ravoid1_collect1): random |
gg_ring (approach-nearest-food) | class_zeroshot (grid weights,
zero ring data) | native_ceiling (ring-fit) | shuffled_class.

Registered predictions (before any run):
  P1 class_zeroshot >= random + 0.5 AND recovers >= 70% of the
     native gain on rcollect1, 6/6 seeds.
  P2 ravoid1_collect1: positive transfer (>= random + 0.5, >= 50%
     of native gain) on >= 5/6 seeds.
  P3 shuffled_class collapses on >= 4/6 seeds (5-dim permutation
     caveat as F267).
"""
