"""EVENT SEMANTICS IN THE TREE: death prunes, consumption removes
(F260).

Four dynamics refutations (F248, F254, F258, F259) re-read F257: the
true simulator's planning advantage is EVENTS, not positions. Every
learned roll lets the tree re-collect the same food at every depth
and lets suicidal branches keep accruing leaf value. This probe adds
the two event rules to F256's transfer planner, priced by the head
it already has:

  DEATH PRUNE   an edge whose predicted typed reward <= -0.4 ends
                the branch: the edge cost is charged, no further
                reward or leaf value accrues.
  CONSUMPTION   after an edge whose positive contact contribution
                >= +0.2 fires for a group, that group is marked
                consumed in the child (moved to the far-corner
                placeholder), so it cannot be re-collected.

Arms: random | transfer_d4 (F256/F257 anchor) | ev_d2 | ev_d4 |
ev_noprune_d4 | ev_noconsume_d4 (ablations) | shufev_d4, on the
trio + control.

Registered predictions (before any run):
  P1 ev_d4 >= transfer_d4 + 0.10 on >= 2/3 trio worlds.
  P2 ablations: each rule contributes on the worlds that exercise
     it (prune on the lethal trio, consume on the control and
     food-rich worlds); either/or logged as found.
  P3 shufev collapses; P4 control world: ev_d4 >= transfer_d4 - 0.1.
"""
...
def plan_event(reference, psi, bank, w_r, w_v, depth,
               prune=True, consume=True):
    """F260: plan_typed plus event semantics. DEAD rows stop
    accruing; consumed groups are parked on the far corner."""
    E = reference.shape[0]
    S = reference.unsqueeze(0)
    ACC = torch.zeros(1, E)
    DEAD = torch.zeros(1, E, dtype=torch.bool)
    FIRST = torch.zeros(1, dtype=torch.long)
    for level in range(depth):
        new_s, new_acc, new_dead, new_first = [], [], [], []
        K = S.shape[0]
        ...
        for act in range(4):
            ...
            r = torch.where(DEAD, torch.zeros_like(r), r)
            child_dead = DEAD | (prune & (r <= -0.4))
            if consume:
                # per-group positive contact: contact-only and both
                # features of group g live at fixed offsets; use the
                # head's own contribution to decide
                child = child.clone()
                for g_ix, (_pl, base) in enumerate(TRACKED):
                    ...
                    got = (contrib.view(K, E) >= 0.2) & ~child_dead
                    got_flat = got.reshape(-1) & ~dead_flat
                    child[got_flat, base] = 7
                    child[got_flat, base + 1] = 7
            new_s.append(child.view(K, E, SLOTS))
            new_acc.append(ACC + r)
            new_dead.append(child_dead)
            new_first.append(FIRST if level > 0
                             else torch.full((K,), act,
                                             dtype=torch.long))
        S = torch.cat(new_s)
        ACC = torch.cat(new_acc)
        DEAD = torch.cat(new_dead)
        FIRST = torch.cat(new_first)
    leafv = (phi(S.reshape(-1, SLOTS)) @ w_v).view(S.shape[0], E)
    leaf = ACC + torch.where(DEAD, torch.zeros_like(leafv), leafv)
...
def play(config, mode, bank, w_r, w_v, seed, depth, soft=False):
    ...
    for _ in range(steps):
        ...
        if mode == "random":
            ...
            action = plan_event(state, tracker.psi(), bank, w_r, w_v,
                                depth,
                                prune="noprune" not in mode,
                                consume="noconsume" not in mode)
        elif mode == "linear":
...
for name, config in TARGET_WORLDS:
    ...
    row["ev_d2"] = play(config, "event", bank, w_tr, w_tv,
                        args.seed * 977, 2)
    row["ev_d4"] = play(config, "event", bank, w_tr, w_tv,
                        args.seed * 977, 4)
    row["ev_noprune_d4"] = play(config, "event_noprune", bank, w_tr,
                                w_tv, args.seed * 977, 4)
    row["ev_noconsume_d4"] = play(config, "event_noconsume", bank,
                                  w_tr, w_tv, args.seed * 977, 4)
    perm = torch.randperm(R_FEATS,
                          generator=torch.Generator().manual_seed(
                              args.seed + 8080))
    row["shufev_d4"] = play(config, "event", bank, w_tr[perm], w_tv,
                            args.seed * 977, 4)
