"""TYPED VALUE TRANSFER: the founding thesis at the value level
(F256).

F255 re-scoped the trio stall: the compound decision function is not
learnable from ~10^4 compound-world samples by any smooth regressor
-- with the tracked 8-slot state OR privileged full state. The
founding objective ("task A makes novel task B faster to learn")
prescribes the response: learn each mechanism's typed value on
SINGLE-MECHANISM worlds, where its function is simple, and carry the
type->value bindings to the compound world. The typed head's
coordinates (event x entity-signature) are world-independent by
construction, so its weights are the transferable object.

TRAIN worlds (single mechanism + minimal hosts):
  collect1 | avoid1_collect1 | pursue1 | intercept1 |
  collect1_resource1 | delayed3
Each contributes mc_soft head rows (its own bank for the rolls);
POOLING the rows and refitting the ridge is the composition.

TARGET worlds: the certified trio + control. The bank (dynamics) is
always fit on the target -- reward-free experience; only reward /
value knowledge transfers.

Arms: random | cold_d4 (head fit on the target rollout only -- the
F253 protocol) | transfer_d4 (pooled simple-world head, ZERO target
reward data) | warm_d4 (pooled simple + target rows) |
shufwarm_d4 (warm weights row-shuffled).

Registered predictions (before any run):
  P1 warm start beats cold at equal target data: warm_d4 >= cold_d4
     + 0.10 on >= 2/3 trio worlds.
  P2 types carry with zero target reward data: transfer_d4 >=
     cold_d4 - 0.10 on >= 2/3 trio worlds.
  P3 shufwarm_d4 collapses relative to warm_d4 everywhere.
  P4 control world: warm does no harm (>= cold - 0.10).
"""
...
for name, config in TARGET_WORLDS:
    ...
    rollout = collect_experience(config, args.seed * 53)
    X, y, S, ret = build_head_data(*rollout, soft=True, bank=bank)
    w_c, w_cv = fit_typed(X, y, S, ret)
    row["cold_d4"] = play(config, "typed", bank, w_c, w_cv,
                          args.seed * 977, 4, soft=True)
    row["transfer_d4"] = play(config, "typed", bank, w_tr, w_tv,
                              args.seed * 977, 4, soft=True)
    w_w, w_wv = fit_typed(torch.cat([lib_X, X]),
                          torch.cat([lib_y, y]),
                          torch.cat([lib_S, S]),
                          torch.cat([lib_ret, ret]))
    row["warm_d4"] = play(config, "typed", bank, w_w, w_wv,
                          args.seed * 977, 4, soft=True)
    perm = torch.randperm(R_FEATS,
                          generator=torch.Generator().manual_seed(
                              args.seed + 8080))
    row["shufwarm_d4"] = play(config, "typed", bank, w_w[perm], w_wv,
                              args.seed * 977, 4, soft=True)
