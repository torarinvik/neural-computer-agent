"""TRANSFER DECOMPOSITION: where does the residual trio gap live
after F256 -- head coverage, dynamics, curriculum, or data policy?
(F257)

F256 broke the deploy wall with typed value transfer (trio -0.75/
-0.55/-0.66 vs ceilings -0.39/-0.27/-0.13). Four registered levers
on the residual, one probe:

  truedyn_transfer_d2  privileged dynamics + the TRANSFERRED head:
                       if this clears transfer_d4 by >= 0.15,
                       DYNAMICS is the residual bottleneck; within
                       0.10, HEAD COVERAGE is. (P1, either/or)
  rich_d4              curriculum enriched with pairwise worlds
                       (pursue1_collect1, intercept1_collect1,
                       delayed3_collect1, avoid1_resource1_collect1)
                       pooled with the six singles: coverage
                       hypothesis. (P2: >= transfer + 0.05 on >= 2/3)
  transfer_it_d4       transfer head + target rows collected UNDER
                       the transfer policy: F256 showed random-
                       policy compound data poisons; is GOOD-policy
                       compound data clean? (P3, either/or: >=
                       transfer + 0.10 -> poisoning is policy-
                       dependent; <= transfer - 0.10 -> compound
                       data poisons regardless)
  shufrich_d4          rich weights row-shuffled (control, P3-style
                       collapse expected everywhere)

P4: control world unharmed by every arm (>= cold - 0.10 equivalent,
measured against transfer_d4 there).
"""
...
RICH_WORLDS = [
    ...
    ("intercept1_collect1", FamilyConfig(intercept=1, collect=1)),
    ("delayed3_collect1", FamilyConfig(delayed=3, collect=1)),
    ("avoid1_resource1_collect1",
     FamilyConfig(avoid=1, resource=1, collect=1)),
]
...
def library_rows(worlds):
    ...
    for name, config in worlds:
        ...
        xs.append(X); ys.append(y); ss.append(S); rets.append(ret)
        print(f"  [train] {name:<28} rows {X.shape[0]}", flush=True)
    return (torch.cat(xs), torch.cat(ys), torch.cat(ss),
            torch.cat(rets))


base_X, base_y, base_S, base_ret = library_rows(TRAIN_WORLDS)
w_tr, w_tv = fit_typed(base_X, base_y, base_S, base_ret)
rich_X, rich_y, rich_S, rich_ret = library_rows(RICH_WORLDS)
w_rr, w_rv = fit_typed(torch.cat([base_X, rich_X]),
                       torch.cat([base_y, rich_y]),
                       torch.cat([base_S, rich_S]),
                       torch.cat([base_ret, rich_ret]))
...
for name, config in TARGET_WORLDS:
    ...
    row["truedyn_transfer_d2"] = play(config, "truedyn_typed", None,
                                      w_tr, w_tv, args.seed * 977, 2,
                                      soft=True)
    row["rich_d4"] = play(config, "typed", bank, w_rr, w_rv,
                          args.seed * 977, 4, soft=True)
    def tr_policy(state, psi):
        return plan_typed(state, psi, bank, w_tr, w_tv, 4, soft=True)
    rollout2 = collect_experience(config, args.seed * 59,
                                  policy=tr_policy)
    X2, y2, S2, ret2 = build_head_data(*rollout2, soft=True, bank=bank)
    w_it, w_itv = fit_typed(torch.cat([base_X, X2]),
                            torch.cat([base_y, y2]),
                            torch.cat([base_S, S2]),
                            torch.cat([base_ret, ret2]))
    row["transfer_it_d4"] = play(config, "typed", bank, w_it, w_itv,
                                 args.seed * 977, 4, soft=True)
    perm = torch.randperm(R_FEATS,
                          generator=torch.Generator().manual_seed(
                              args.seed + 8080))
    row["shufrich_d4"] = play(config, "typed", bank, w_rr[perm],
                              w_rv, args.seed * 977, 4, soft=True)
