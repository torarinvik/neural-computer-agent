"""CROSS-SUBSTRATE TYPED VALUE TRANSFER: amodal value knowledge
(F265).

The typed head's coordinates -- contact/teleport events x
motion/approach signatures -- are defined by DISTANCE alone, not by
grid geometry. This probe fits the head on the six simple GRID
worlds (true-transition fit; no plant needed for the measurement)
and applies the SAME WEIGHTS on the GRAPH substrate (F241: 8 nodes,
4 ports, nominal identity, BFS metric), zero graph experience of
any kind. Graph events: contact = BFS distance kernel, teleport =
BFS jump > 2 between an entity's consecutive nodes, boundary = 0
(no graph analog; its weights sit inert). The avatar's one-step
dynamics are the edge relation -- part of the substrate's percept
(F241 structure()); entities roll frozen.

Arms per graph world: random | gg_bfs (approach-nearest-food
baseline, the F241-style form) | typed_zero (grid weights, ZERO
graph data) | typed_graphfit (same features refit on graph rollouts
-- the native ceiling for this head class) | shuffled.

Registered predictions (before any run):
  P1 amodal transfer: typed_zero >= random + 0.5 AND >= shuffled
     + 0.5 on both collect-containing graph worlds.
  P2 near-native: typed_zero >= 0.7 x typed_graphfit on >= 2/3
     graph worlds (in gains over random).
  P3 shuffled ~ random everywhere.
"""
...
# ---- grid library head (true-transition fit, no plant needed) ----
TRAIN_WORLDS = [
...
GRAPH_WORLDS = [
    ...
    ("gavoid1_collect1", GraphConfig(collect=1, avoid=1)),
    ("gcollect2", GraphConfig(collect=2)),
]

report["results"] = {}
for name, config in GRAPH_WORLDS:
    row = {}
    row["random"] = play_graph(config, "random", None,
                               args.seed * 977)
    row["gg_bfs"] = play_graph(config, "gg_bfs", None,
                               args.seed * 977)
    ...
    perm = torch.randperm(R_FEATS,
                          generator=torch.Generator().manual_seed(
                              args.seed + 8080))
    row["shuffled"] = play_graph(config, "typed", w_tr[perm],
                                 args.seed * 977)
