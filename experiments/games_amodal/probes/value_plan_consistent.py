"""MODEL-CONSISTENT typed head x graded contact: fixing the
event/dynamics interface (F253).

F252 validated the typed event head over true dynamics and refuted
its composition with the learned bank: a hard contact predicate
(d <= 1) flips under one-cell model error, and shuffled weights beat
the fitted head on the lethal worlds -- precisely-wrong advice is
worse than noise. Two orthogonal, fully-deployable fixes, run as a
2x2 against F252's pure arm:

  MC   model-consistent training: fit the head on (parent,
       BANK-ROLLED child) pairs -- the head learns to read reward
       through the model's own systematic biases, because training
       and planning share the representation.
  SOFT graded contact: the binary d<=1 predicate becomes the kernel
       clamp((2 - d)/2, 0, 1) -- +-1 model error degrades the score
       instead of flipping it.

Everything else (tracker, signatures, death attribution, planner,
linear V at leaves) is F252's, unchanged.

Arms: random | linear_d4 (F250 head) | typed_d4 (F252 replication)
| soft_d4 | mc_d4 | mc_soft_d4 | mc_soft_it_d4 (policy iteration)
| shuffled_mc_soft_d4.

Registered predictions (before any run):
  P1 the interface is the defect: the best MC arm >= midpoint
     between linear_d4 and F252's truedyn_typed_d2 (-0.62/-0.48/
     -0.89) on >= 2/3 trio worlds.
  P2 factor attribution (either/or, registered): if MC alone
     recovers and SOFT alone does not, consistency was the active
     ingredient; the reverse indicts predicate brittleness alone;
     both-only indicts the pair. Logged as found.
  P3 shuffled_mc_soft_d4 <= random + 0.1 (fitted advice must beat
     noise again -- the F252 inversion must disappear).
  P4 control world: best MC arm >= linear_d4 - 0.1.
"""
...
for name, config in WORLDS:
    ...
    row["linear_d4"] = play(config, "linear", bank, w_lr, w_lv,
                            args.seed * 977, 4)
    # the 2x2: {true-fit, model-consistent} x {hard, soft}
    heads = {}
    for label, soft, use_bank in (("typed", False, None),
                                  ("soft", True, None),
                                  ("mc", False, bank),
                                  ("mc_soft", True, bank)):
        X, y, S, ret = build_head_data(*rollout, soft=soft,
                                       bank=use_bank)
        heads[label] = (fit_typed(X, y, S, ret), (X, y, S, ret), soft)
        (w_r, w_v), _, _ = heads[label]
        row[f"{label}_d4"] = play(config, "typed", bank, w_r, w_v,
                                  args.seed * 977, 4, soft=soft)
    (w_r, w_v), (X, y, S, ret), _ = heads["mc_soft"]
    def mc_policy(state, psi):
        return plan_typed(state, psi, bank, w_r, w_v, 4, soft=True)
    ...
    row["mc_soft_it_d4"] = play(config, "typed", bank, w_r2, w_v2,
                                args.seed * 977, 4, soft=True)
    perm = torch.randperm(R_FEATS,
                          generator=torch.Generator().manual_seed(
                              args.seed + 8080))
    row["shuffled_mc_soft_d4"] = play(config, "typed", bank,
                                      w_r[perm], w_v,
                                      args.seed * 977, 4, soft=True)
