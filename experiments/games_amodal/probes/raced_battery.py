"""RACED BATTERY: adaptive racing vs the flat selection protocol.

The selection-optimism ledger spans goals, schedules, guards and
encoders (F231-F236), and in F235/F236 it demonstrably cost
performance: encoders won selection then collapsed on evaluation.
This probe replaces the flat min-of-two-streams protocol with
ADAPTIVE RACING (fast_stack.race): common-random-number stages --
every candidate faces identical tiled worlds -- with staged budgets
(16 -> 48 -> 96 episodes) and elimination of dominated candidates.

Two arms per world, identical candidate spaces (singles + composites
+ schedules across all four encoders):
  flat    the F236-era protocol: min over two 48x12 streams
  raced   fast_stack.race with CRN stages

Registered predictions:
  1. raced >= flat on evaluation, pooled over the battery (paired per
     seed x world); the deceptive encoder-collapse cases fix
     themselves (second2 beats clear/approach when judged on pooled
     CRN evidence).
  2. raced spends FEWER candidate-episodes than flat (elimination
     beats uniform allocation).
  3. Winner optimism (selection estimate - fresh evaluation) shrinks
     under racing.
"""
...
def choose_by_race_adaptive(config, bank, label):
    """Same candidate space as choose_by_race, selected by adaptive
    CRN racing instead of flat min-of-two-streams."""
    ...
    schedules = [((legs[i],), (legs[j],))
                 ...
                 if i != j]
    fenc = FAST_ENC[label] if label in FAST_ENC else         globals()["FAST_ENC_OVERRIDE"]

    def crn_factory(episodes, seed):
        def factory(batch):
            base = FastFamilyVerifier(config, batch_size=episodes,
                                      seed=seed)
            base.reset(seed=seed)
            HOOK["v"] = FastHookAdapter(base)
            return tile_verifier(base, batch // episodes)
        return factory

    def scorer_for(scorefn):
        def scorer(sub_items, episodes, seed):
            return scorefn(crn_factory(episodes, seed), sub_items, bank,
                           plant_executor, fenc, episodes,
                           args.search_steps, seed)
        return scorer

    stages = [(16, args.seed * 977 + 11), (48, args.seed * 977 + 12),
              (96, args.seed * 977 + 13)]
    # rank singles once on the cheap CRN stage to build the composite
    # pool from the TOP singles, then race everything properly
    s1 = scorer_for(score_goals)(singles, *stages[0])
    top16 = [singles[int(i)] for i in
             s1.argsort(descending=True)[:16]]
    goal, score, spent = race(singles, scorer_for(score_goals), stages)
    spent += stages[0][0] * len(singles)
    comp = [(a[0], b[0]) for a in top16 for b in top16 if a != b]
    best_goal, best_score = goal, score
    if comp:
        g2, s2, sp2 = race(comp, scorer_for(score_goals), stages)
        spent += sp2
        if s2 > best_score:
            best_goal, best_score = g2, s2
    if schedules:
        g3, s3, sp3 = race(schedules, scorer_for(score_schedules),
                           stages)
        spent += sp3
        if s3 > best_score:
            best_goal, best_score = ("schedule", list(g3)), s3
    return best_goal, best_score, spent
...
for name, config in WORLDS:
    ...
    for arm in ("flat", "raced"):
        ...
        for label, candidate in ENC_CANDIDATES.items():
            ...
            if arm == "flat":
                ...
                if picked is None:
                    continue
                goal, score = picked
                spent = 2 * args.search_episodes * 500  # flat ~ uniform
            else:
                picked = choose_by_race_adaptive(config, banks[label],
                                                 label)
                if picked is None:
                    continue
                goal, score, spent = picked
            spent_total += spent
        ...
        bank_score = play(config, "bank", bank, args.seed * 977, goal,
                          plant_executor, args.episodes, args.steps)
        row[arm] = {"encoder": label, "goal": shown,
                    "selection": round(sel_score, 4),
                    "optimism": round(sel_score - bank_score, 4),
                    "episodes_spent": spent_total,
                    "bank": bank_score}
    globals()["enc"] = ENC_CANDIDATES["second2"]
    row["random"] = play(config, "random", None, args.seed * 977, None,
                         None, args.episodes, args.steps)
    results[name] = row
    cells = "  ".join(f"{a} {row[a]['bank']:+.3f}(opt {row[a]['optimism']:+.2f})"
                      for a in ("flat", "raced") if a in row)
    print(f"  {name:<16} rnd {row['random']:+.3f}  {cells}", flush=True)
