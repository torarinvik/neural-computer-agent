"""TRANSFER MATRIX: the founding curve at the mechanism level (F238).

F219 measured the founding objective across parameter variants of one
family. The stack now owns mechanism-level capabilities (SEQUENCE,
VALUE-BIND + entity identity), so the question is re-asked one level
up, with cost charged honestly: does experience with mechanisms make
NOVEL mechanism combinations cheaper to acquire?

Five novel worlds from the DEV vocabulary (no sealed components), two
acquisition arms with the same architecture available:
  cold  full adaptive race (all four standard encoders) plus the
        valued-canonical set with its value fit -- discovery from
        scratch.
  warm  race over a ~8-form incumbent library transferred from the
        DEV originals -- the resource1 schedule form, the deceptive
        valued-REACH forms, approach/avoid/composite forms, and a
        schedule of valued legs that no world has ever selected.
Costs count race episodes AND value-fit episodes for both arms.

Registered predictions:
  1. warm >= 95% of cold return at <= 25% of cold episode cost,
     pooled over the five worlds.
  2. resource1_deceptive1 (double compound) is solved warm via
     COMPOSITION of transferred forms.
  3. The winner table is a transfer matrix: same-mechanism targets
     reuse their source form; compounds cross them.
"""
...
SCHED_VAL = ("schedule", [APPROACH2, VALUED])

WARM_LIBRARY = [
    ("plain", APPROACH1), ("plain", APPROACH2), ("plain", AVOID2),
    ("plain", COMP_A1AV), ("plain", SCHED_RES),
    ("valued", VALUED), ("valued", VALUED_AV), ("valued", SCHED_VAL),
]

VALUE_FIT_EPISODES = 10 * 128  # fit_value_w rollout cost, counted
...
for name, config in WORLDS:
    ...
    valued_enc = (make_valued_enc("learned", w_learned)
                  if w_learned is not None else None)
    ...
    # ---- WARM: race the incumbent library ----
    spent = VALUE_FIT_EPISODES if valued_enc is not None else 0
    scored = []
    for kind, goal in WARM_LIBRARY:
    ...
    enc_fn = valued_enc if kind == "valued" else enc_std
    bank = bank_val if kind == "valued" else bank_std
    row["warm"] = {"form": kind, "goal": str(goal),
                   "episodes_spent": spent,
                   "bank": eval_goal(config, bank, goal, enc_fn)}

    # ---- COLD: full adaptive race + valued canonical set ----
    spent = 0
    best = None
    for label, candidate in ENC_CANDIDATES.items():
        ...
        if best is None or score_c > best[0]:
            best = (score_c, label, goal_c, bank_l, candidate)
    if valued_enc is not None:
        spent += VALUE_FIT_EPISODES
        for goal_c in (VALUED, VALUED_AV):
            s = min(score_slow(config, bank_val, goal_c, valued_enc, 48,
                               args.seed * 977 + off) for off in (1, 2))
            spent += 2 * 48
            if best is None or s > best[0]:
                best = (s, "valued_learned", goal_c, bank_val,
                        valued_enc)
    score_c, label, goal_c, bank_c, enc_c = best
    row["cold"] = {"form": label, "goal": str(goal_c),
                   "episodes_spent": spent,
                   "bank": eval_goal(config, bank_c, goal_c, enc_c)}
    globals()["enc"] = enc_std
    row["random"] = play(config, "random", None, args.seed * 977, None,
                         None, args.episodes, args.steps)
    results[name] = row
    print(f"  {name:<20} rnd {row['random']:+.3f}  warm "
          f"{row['warm']['bank']:+.3f} ({row['warm']['episodes_spent']}"
          f"ep, {row['warm']['form']})  cold {row['cold']['bank']:+.3f}"
          f" ({row['cold']['episodes_spent']}ep, {row['cold']['form']})",
          flush=True)
