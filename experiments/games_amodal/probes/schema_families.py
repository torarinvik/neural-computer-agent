...
def random_family_spec(generator: torch.Generator, cap: int = 512,
    ...
    if wide and slots >= 2:
        kinds.append("pair")
    if gated:
        # F92: the reacher's WALLED grid costs 438 updates against a cold
        # 88 -- the mechanism's only decisive failure. Every op above is
        # a uniform function of slot VALUES, while a wall makes an
        # action's effect depend on WHICH STATE you are in. These two
        # primitives supply that missing kind:
        #   wall  a barrier on one axis -- the move is refused when it
        #         would land on a particular value, which is literally
        #         what the reacher's obstacle does;
        #   cond  an effect that fires only when ANOTHER slot is
        #         non-zero, i.e. an action whose consequence depends on
        #         context rather than on its own coordinate.
        kinds.append("wall")
        if slots >= 2:
            kinds.append("cond")
    if space == "perm":
...
def apply_op(state: tuple, op: tuple, values: int) -> tuple:
    ...
    if kind == "swap":
        ...
        return tuple(out)
    if kind == "wall":
        # a barrier: the move is refused when it would land on this value
        _, slot, barrier = op
        moved = min(values - 1, out[slot] + 1)
        if moved == barrier:
            return tuple(out)
        out[slot] = moved
        return tuple(out)
    if kind == "cond":
        # fires only when the OTHER slot is non-zero: the effect depends
        # on context, not on the acting coordinate
        _, slot, other = op
        if out[other] != 0:
...
# A family with NO RULE at all: its transition table is a random
# permutation per action, so nothing about it is compressible. F97's
# exception store grows only where rules fail -- this is the family
# where the rule fails EVERYWHERE, and it is the degeneration case that
# must be measured rather than assumed away. A store that behaves well
# here stays bounded and reports that it is not coping; a store that
# behaves badly silently becomes the whole transition table.
_CHAOS = torch.Generator().manual_seed(4242)
_CHAOS_TABLE = [[int(col[row]) for col in
                 [torch.randperm(64, generator=_CHAOS) for _ in range(4)]]
                for row in range(64)]


def chaos_step(state, action):
    return chaos_states()[_CHAOS_TABLE[state[0] * 8 + state[1]][action]]


FAMILIES["chaos"] = {"slots": 2, "values": 8, "actions": 4,
                     "states": chaos_states, "step": chaos_step}
