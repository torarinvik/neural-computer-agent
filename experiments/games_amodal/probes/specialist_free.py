"""SPECIALIST-FREE CONTROL ON THE HOME SUBSTRATE: value-directed
navigation replaces the hand-written goal grammar (F273).

F272 showed transferred values + the substrate's metric + one raced
risk scalar reach specialist parity on ring and graph. Every grid
result in this project, by contrast, still rests on a HAND-WRITTEN
goal library (approach1/approach2/avoid2/comp -- forms a person
chose). This probe removes them at home:

    score(a) = sum_g w_g * v_hat(psi_g) * (d_now(g) - d_after(g)),
    w_g = 1 if v_hat >= 0 else beta * proximity(d_now)

with the avatar rolled by its plant-executed bank program (the
gradient is over the plant's own predicted move), values
transferred from the six simple worlds' class binding, and
(beta, lambda_pooled) raced on the selection stream -- optionally
adding the pooled event head as a second term.

Arms: random | goal_grammar (hand-written specialist) |
hybrid_raced (F264/F268 frontier: hand-written goal + pooled head)
| nav_raced (specialist-free) | nav_shuf_raced (control).

Registered predictions (before any run):
  P1 specialist-free parity: nav_raced >= goal_grammar - 0.15 on
     >= 4/6 worlds at the mean.
  P2 frontier parity on the certified trio: nav_raced >=
     hybrid_raced - 0.15 on >= 2/3 trio worlds.
  P3 nav_shuf_raced collapses (>= 0.3 below nav_raced) on >= 4/6
     worlds.
  P4 the raced beta is reported per world (risk economics of the
     grid mechanisms, comparable to F272's ring/graph values).
"""
...
def play_nav(config, bank, seed, beta, lam_pooled=0.0,
             w_class=None):
    """Specialist-free control: transferred values x the grid's own
    Manhattan gradient over the plant-rolled avatar move."""
    episodes, steps = args.episodes, args.steps
    v = FamilyVerifier(config, batch_size=episodes, seed=seed)
    v.reset(seed=seed)
    tracker = Tracker(episodes)
    ...
    for _ in range(steps):
        ...
        for act in range(4):
            ...
            score = torch.zeros(episodes)
            for g_ix, (_pl, base) in enumerate(TRACKED):
                d0 = ((reference[:, 0] - reference[:, base]).abs()
                      + (reference[:, 1]
                         - reference[:, base + 1]).abs()).float()
                d1 = ((child[:, 0] - reference[:, base]).abs()
                      + (child[:, 1]
                         - reference[:, base + 1]).abs()).float()
                val = psi_now[:, g_ix] @ w_class
                near = ((3.0 - d0) / 3.0).clamp(0.0, 1.0)
                val = torch.where(val < 0, val * beta * near, val)
                score = score + val * (d0 - d1)
            if lam_pooled:
                x = event_features(reference, child,
                                   torch.full((episodes,), act,
                                              dtype=torch.long),
                                   psi_now, soft=True)
                score = score + lam_pooled * (x @ w_tr)
            if best is None:
                best = score.clone()
            else:
                take = score > best
...
for name, config in TARGET_WORLDS:
    ...
    row["shufclass_h1"] = play_hybrid(config, bank, LIBRARY[3][1],
                                      ...
                                      head="class",
                                      w_class=w_cl[permc])
    # F273: specialist-free navigation, (beta, lambda) raced
    NAV = [(b, l) for b in (0.0, 0.25, 0.5, 1.0, 2.0, 4.0)
           for l in (0.0, 0.5)]

    def race_nav(w):
        sel = [play_nav(config, bank, args.seed * 977 + 1, b, l, w)
               for b, l in NAV]
        return NAV[max(range(len(NAV)), key=lambda i: sel[i])]

    b_n, l_n = race_nav(w_cl)
    row["nav_raced"] = play_nav(config, bank, args.seed * 977, b_n,
                                l_n, w_cl)
    row["nav_beta"] = b_n
    row["nav_lam"] = l_n
    b_s, l_s = race_nav(w_cl[permc])
    row["nav_shuf_raced"] = play_nav(config, bank, args.seed * 977,
                                     b_s, l_s, w_cl[permc])
