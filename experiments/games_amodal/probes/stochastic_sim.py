"""STOCHASTIC PLAN-TIME SIMULATION: sampled futures with realized
events (F262).

The F261-completed necessity case, cashed in. The true simulator's
certified advantage (F257: true d2 beats every learned d4) lives in
SAMPLED, EVENT-REALIZED futures -- not in one modal composition.
This build stores, per (signature bucket, entity cell, avatar cell)
key, the EMPIRICAL DELTA DISTRIBUTION (reservoir of observed
deltas), and plans by Monte-Carlo: for each first action, M sampled
rollouts to depth d; the avatar rolls through its plant-executed
bank program, entities roll by SAMPLING their distribution, the
continuation policy is the greedy 1-step typed head, and events are
realized PER SAMPLE -- an edge priced <= -0.4 terminates that
sample, a group contribution >= +0.2 parks that group (consumed).
Score = mean over samples of accrued typed reward + leaf V (alive
samples only). All learned, all external memory; the transferred
F256 head prices everything.

Arms: random | transfer_d4 (anchor) | sim_d2 | sim_d4 (M=12) |
sim_modal_d4 (ablation: distributions collapsed to their mode --
isolates stochasticity as the ingredient) | shufsim_d4.

Registered predictions (before any run):
  P1 sim_d4 >= transfer_d4 + 0.10 on >= 2/3 trio worlds (the
     certified prize is finally collected by a deployable stack).
  P2 stochasticity is the active ingredient: sim_d4 >= sim_modal_d4
     + 0.05 on >= 2/3 trio worlds.
  P3 shufsim collapses; P4 control world: sim_d4 >= transfer_d4
     - 0.10.
"""
...
def psi_bucket(psi):
    """[B, 3] signature bucket per tracked entity: mover bit (motion
    energy > 0.3) x approacher bit (approach fraction > 0.5)."""
    return ((psi[:, :, 3] > 0.3).long() * 2
            + (psi[:, :, 4] > 0.5).long())


def build_table(raw, psis, alive):
    ...
    full = [Counter() for _ in range(4 * 3 * GRID ** 4)]
    marg = [Counter() for _ in range(4 * 3 * GRID ** 2)]
    T = len(raw) - 1
    for t in range(T):
        ...
        for g, (_pl, base) in enumerate(TRACKED):
            ...
            for i in valid.nonzero().flatten().tolist():
                ...
                if abs(dr) + abs(dc) > 4 and abs(dr) < 6:
                    continue
                gb = int(buckets[i, g]) * 3 + g
                kf = ((gb * GRID + int(er[i])) * GRID + int(ec[i]))
                full[(kf * GRID + int(ar[i])) * GRID + int(ac[i])][
                    (dr, dc)] += 1
                marg[kf][(dr, dc)] += 1
    RES = 8
    TF = torch.zeros(4 * 3 * GRID ** 4, RES, 2, dtype=torch.long)
    NF = torch.zeros(4 * 3 * GRID ** 4, dtype=torch.long)
    for k, c in enumerate(full):
        if c:
            ds = list(c.elements())[:RES]
            for i, d in enumerate(ds):
                TF[k, i, 0], TF[k, i, 1] = d
            NF[k] = len(ds)
    TM = torch.zeros(4 * 3 * GRID ** 2, RES, 2, dtype=torch.long)
    NM = torch.zeros(4 * 3 * GRID ** 2, dtype=torch.long)
...
def table_roll(parent, av_r, av_c, table, buckets, g_rand=None,
               modal=False):
    """Entities via the type-keyed transition DISTRIBUTION: sample
    an observed delta (g_rand set), or take entry 0 approximating
    the mode (modal ablation); marginal fallback, FROZEN default."""
    TF, NF, TM, NM = table
    child = parent.clone()
    ...
    for g, (_pl, base) in enumerate(TRACKED):
        ...
        gb = buckets[:, g] * 3 + g
        kf = ((gb * GRID + er) * GRID + ec)
        kfull = (kf * GRID + av_r) * GRID + av_c
        nf, nm = NF[kfull], NM[kf]
        if modal or g_rand is None:
            pick_f = torch.zeros(N, dtype=torch.long)
            pick_m = torch.zeros(N, dtype=torch.long)
        else:
            u = torch.rand(N, generator=g_rand)
            pick_f = (u * nf.clamp(min=1).float()).long()
            pick_m = (u * nm.clamp(min=1).float()).long()
        df = TF[kfull, pick_f]
        dm = TM[kf, pick_m]
        d = torch.where((nf > 0).unsqueeze(1), df,
                        torch.where((nm > 0).unsqueeze(1), dm,
                                    torch.zeros_like(df)))
        nr, nc = er + d[:, 0], ec + d[:, 1]
...
def model_child_mem(parent, acts, bank, table, buckets):
    child = parent.clone()
    for a in range(4):
        rows = acts == a
        program = bank.get(a)
...
def plan_mc(reference, psi, bank, table, w_r, w_v, depth,
            samples, g_rand, modal=False):
    """Monte-Carlo planning: depth-1 exhaustive over first actions,
    then a greedy 1-step typed-head continuation policy inside each
    sampled future; events realized per sample."""
    E = reference.shape[0]
    buckets = psi_bucket(psi)
    score = torch.zeros(4, E)
    for act in range(4):
        acc = torch.zeros(samples * E)
        alive = torch.ones(samples * E, dtype=torch.bool)
        S = reference.repeat(samples, 1)
        psi_t = psi.repeat(samples, 1, 1)
        buck_t = buckets.repeat(samples, 1)
        a_now = torch.full((samples * E,), act, dtype=torch.long)
        for level in range(depth):
            child = S.clone()
            for a in range(4):
                rows = (a_now == a) & alive
                program = bank.get(a)
                if program is not None and bool(rows.any()):
                    child[rows] = plant_executor(program, S[rows])
            child = table_roll(S, child[:, 0], child[:, 1], table,
                               buck_t, g_rand=g_rand, modal=modal)
            x = event_features(S, child, a_now, psi_t, soft=True)
            r = x @ w_r
            acc = acc + torch.where(alive, r, torch.zeros_like(r))
            died = alive & (r <= -0.4)
            # consumption: park groups whose contribution fired
            for g_ix, (_pl, base) in enumerate(TRACKED):
                off = 1 + g_ix * EVENTS * PSI_DIM
                contrib = (x[:, off:off + EVENTS * PSI_DIM]
                           @ w_r[off:off + EVENTS * PSI_DIM])
                got = alive & (contrib >= 0.2)
                child[got, base] = 7
                child[got, base + 1] = 7
            alive = alive & ~died
            S = child
            if level + 1 < depth:
                # greedy 1-step continuation: pick next action by
                # immediate typed reward over a modal peek
                best_r, a_next = None, torch.zeros_like(a_now)
                for a in range(4):
                    program = bank.get(a)
                    peek = S if program is None else plant_executor(
                        program, S)
                    peek = table_roll(S, peek[:, 0], peek[:, 1],
                                      table, buck_t, modal=True)
                    xr = event_features(S, peek,
                                        torch.full_like(a_now, a),
                                        psi_t, soft=True) @ w_r
                    if best_r is None:
                        best_r = xr.clone()
                    else:
                        take = xr > best_r
                        best_r = torch.where(take, xr, best_r)
                        a_next = torch.where(take,
                                             torch.full_like(a_now, a),
                                             a_next)
                a_now = a_next
        leaf = phi(S) @ w_v
        total = acc + torch.where(alive, leaf, torch.zeros_like(leaf))
        score[act] = total.view(samples, E).mean(dim=0)
    return score.argmax(dim=0)


def play_mc(config, bank, table, w_r, w_v, seed, depth, samples,
            modal=False):
    E, steps = args.episodes, args.steps
    v = FamilyVerifier(config, batch_size=E, seed=seed)
    v.reset(seed=seed)
    g_rand = torch.Generator().manual_seed(seed + 606)
    tracker = Tracker(E)
    total = torch.zeros(E)
    for _ in range(steps):
        code = tracker.encode(frame_cells(v.observation()))
        action = plan_mc(clamp_state(code), tracker.psi(), bank,
                         table, w_r, w_v, depth, samples, g_rand,
                         modal=modal)
        total += v.step(action).reward
    return float(total.mean())
...
# library: F256 bank-consistent transferred head (the anchor stack)
...
for name, config in TRAIN_WORLDS:
    ...
    lib_X.append(X); lib_y.append(y); lib_S.append(S)
    lib_ret.append(ret)
    print(f"  [train] {name:<24} rows {X.shape[0]}", flush=True)
w_tr, w_tv = fit_typed(torch.cat(lib_X), torch.cat(lib_y),
...
for name, config in TARGET_WORLDS:
    ...
    table = build_table(raw, psis, alive)
    row["transfer_d4"] = play(config, "typed", bank, w_tr, w_tv,
                              args.seed * 977, 4, soft=True)
    row["sim_d2"] = play_mc(config, bank, table, w_tr, w_tv,
                            args.seed * 977, 2, 12)
    row["sim_d4"] = play_mc(config, bank, table, w_tr, w_tv,
                            args.seed * 977, 4, 12)
    row["sim_modal_d4"] = play_mc(config, bank, table, w_tr, w_tv,
                                  args.seed * 977, 4, 12, modal=True)
    perm = torch.randperm(R_FEATS,
                          generator=torch.Generator().manual_seed(
                              args.seed + 8080))
    row["shufsim_d4"] = play_mc(config, bank, table, w_tr[perm],
                                w_tv, args.seed * 977, 4, 12)
