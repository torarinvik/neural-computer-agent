"""PURSUIT: distinct ROLES on one plane -- the width-payoff world.

F226 closed the width question for the old family by elimination:
distinct positions (density) and distinct urgencies (intercept) both
collapse to nearest-tracking under a competent planner. What remains is
distinct ROLES: two objects on the SAME plane, rendered identically,
whose difference is visible only in their dynamics. The new `pursue`
family component provides it: a plane-2 object that steps toward the
avatar every turn, next to `avoid` hazards that bounce obliviously.
No single-frame encoder can say which plane-2 object is the threat.

The probe adds ONE generic temporal reduction to the encoder candidate
set: `approaching(plane)` -- match each newly-occupied cell of a plane
to its nearest newly-vacated cell (one-step optical flow) and keep the
nearest cell whose move REDUCED its distance to the avatar. A looming
detector: no object identities, no game semantics, computable from two
consecutive frames. Slots 6/7 carry either this (encoder `approach2`)
or F225's second-nearest (`second2`), chosen per world by planning
selection as always. Goal language is the pair language of record;
selection is F226's robust protocol (min over two disjoint streams,
48x12 rollouts). Arms as in F226: six = goal may name slots 0-5,
eight = all slots; one plant, one bank per (world, encoder).

Worlds: pursue1 (pursuer alone -- six-slot flee suffices, control),
avoid2 (anchor, archived so4 numbers), pursue1_avoid1 and
pursue1_avoid2 (the roles worlds), pursue1_collect1 (flee-and-eat).

Registered predictions:
  1. pursue1: the six arm solves it (the nearest plane-2 object IS the
     pursuer); eight adds ~nothing.
  2. pursue1_avoid1 / pursue1_avoid2: the eight arm picks `approach2`,
     names slots 6/7 with flee sign against the avatar, and beats the
     six arm (which must flee the nearest plane-2 object -- often the
     oblivious bouncer -- while the pursuer closes). Pooled over the
     two roles worlds: t >= +2. This would be the FIRST width payoff,
     and it should arrive only when the roles differ.
  3. avoid2: unchanged vs the archived so4 runs (nothing approaches
     persistently; approach2 either inadmissible or unhelpful).
  4. F223 discipline: the eight advantage must hold on the evaluation
     stream (seed*977), not merely the selection streams.
"""
