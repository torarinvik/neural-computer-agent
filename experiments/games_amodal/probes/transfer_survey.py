"""TRANSFER SURVEY: the F256 stack meets the full generated space
(F263).

The trio residual is closed (F262); the frontier returns to
breadth. This probe deploys mechanism-typed value transfer -- the
six-singles library head, target-fit banks, mc_soft planner --
across ALL 20 GEN-DEV worlds, guarded: per world we race the
transfer planner against the F245-era goal-grammar deploy (the
canonical import library + fallback score recorded per world) by
playing both and reporting each; the guard rule (keep the better)
is applied per world.

Registered predictions (before any run):
  P1 the transfer stack beats the goal-grammar deploy on >= 8/20
     worlds by >= +0.10 (breadth value of the new capability).
  P2 guarded-best >= goal-grammar deploy everywhere (the F243
     safety promise extends to the new form).
  P3 on the five F245 witness worlds, guarded-best improves the
     mean by >= +0.10 (the witnesses were the point).
"""
...
TARGET_WORLDS = [(name_of(c), FamilyConfig(**c)) for c in _dev]
...
for name, config in TARGET_WORLDS:
    ...
    try:
        ...
        report["results"][name] = {"error": str(err)}
        continue
    gg = max(play_goal(config, bank, goal, args.seed * 977)
             for _label, goal in LIBRARY)
    row["goal_grammar"] = gg
    row["transfer_d4"] = play(config, "typed", bank, w_tr, w_tv,
                              args.seed * 977, 4, soft=True)
    row["guarded"] = max(row["goal_grammar"], row["transfer_d4"],
                         row["random"])
    win = row["transfer_d4"] >= row["goal_grammar"] + 0.10
    wins += win
    report["results"][name] = row
    print(f"  {name:<44} rnd {row['random']:+.3f}  gg {gg:+.3f}  "
          f"tr {row['transfer_d4']:+.3f}{'  <== WIN' if win else ''}",
          flush=True)
report["wins"] = wins
print(f"  transfer wins (>= gg+0.10): {wins}/{len(TARGET_WORLDS)}",
      flush=True)
