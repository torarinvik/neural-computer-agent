"""HYBRID CONTROLLER: goal-grammar gradient + typed event pricing
in one action rule (F264).

F263's complementarity law: the goal grammar owns dense collection,
typed transfer owns lethal mechanisms, and the certified trio
worlds contain BOTH structures -- each stack alone is partial
exactly there. This probe composes the two VALUES (not the
dynamics -- that fence stands): the action rule becomes

    argmin_a  goal_cost(child_a) - lambda * r_typed(parent, child_a)

with (goal, lambda) raced per world on a selection stream (library
goals x lambda in {0.5, 1, 2}, plus the pure endpoints), final
evaluation on the standard stream. Both terms are computed on the
same plant-rolled child; the transferred F256 head prices events.

Registered predictions (before any run):
  P1 the intersection prize: hybrid >= max(goal-grammar, transfer)
     + 0.10 on >= 2/3 trio worlds.
  P2 racing safety: hybrid-raced >= max(gg, transfer) - 0.05 on
     every world measured (the race may pick a pure endpoint).
  P3 dense worlds (avoid1_collect1, avoid2_collect1_delayed5):
     hybrid ~ goal-grammar (no harm where one structure dominates).
  P4 intercept2: hybrid keeps the transfer win (>= -0.20).
"""
...
def play_hybrid(config, bank, goal, lam, seed):
...
for name, config in TARGET_WORLDS:
    ...
    # race hybrid candidates on the selection stream
    cands = [(goal, lam) for _l, goal in LIBRARY
             for lam in (0.5, 1.0, 2.0)]
    cands += [(None, 1.0)] + [(goal, None) for _l, goal in LIBRARY]
    sel = [play_hybrid(config, bank, g, l, args.seed * 977 + 1)
           for g, l in cands]
    g_best, l_best = cands[max(range(len(cands)),
                               key=lambda i: sel[i])]
    row["hybrid_raced"] = play_hybrid(config, bank, g_best, l_best,
                                      args.seed * 977)
    row["picked"] = f"lam={l_best}" + ("" if g_best else " pure-tr")
