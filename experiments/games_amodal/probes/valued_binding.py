"""VALUED TARGET BINDING: the F233 capability, factorially localized.

The aliasing audit proved the stack's target token carries zero value
information (L1 = 0.500). This probe adds the minimal typed-controller
element: a VALUE-DIRECTED TRACKER -- slots 6/7 hold the position of
the argmax-value plane-1 item, V_w(o) = w . phi(o), phi(o) a generic
relational token [1, d(o, av), d_manh(o, plane2), d_row(o, plane2)]
-- and the ordinary goal REACH((0,1) -> (6,7)) does the rest. Nothing
names food, bait or hazards; w is world-specific bank content.

Factorial arms (per world, separate races, GPT-advised):
  none      the F233 protocol minus guards (in-run baseline)
  true      PRIVILEGED: the tracker points at an actual food item
            (verifier-private labels) -- binding-sufficiency ceiling
  learned   w ridge-fit from the system's OWN contact experience
            (identity-free phi: audit predicts a ~0.84 ranking cap)
  shuffled  learned w with permuted entries -- causal control

Registered predictions:
  1. true: the tax turns into a bonus or near it (binding via a
     tracker slot is sufficient once values are known).
  2. learned: recovers a large fraction of the true arm (phi contains
     the L2+L3 features; ranking cap ~0.84 predicts partial).
  3. shuffled: no better than none (kills the lottery explanation).
  4. resource1 keeps its schedule (+0.59) in every arm; collect2 and
     avoid1_collect1 unchanged (the valued tracker is admissible but
     should not displace the working goals).
Metrics beyond return: held-out pairwise ranking accuracy of w, and
target agreement between learned and true trackers.
"""
