"""PLAN DEPTH FACTORIAL: localize WHY depth-2 execution lost.

plan_depth.py (archived pd runs) filled two cells: goals selected at
depth 1, executed at depth 1 (baseline) and at depth 2 with CUMULATIVE
aggregation c(s1) + min_b c(s2). Depth 2 lost everywhere (ALL t=-3.05,
pursue t=-3.30). Four explanations are confounded, so this probe runs
the crossed design (user-endorsed localization discipline):

  aggregation : cumulative (archived) vs LEAF-ONLY min_b c(s2) --
                cumulative punishes temporarily-worse intermediate
                states, an artifact of the scoring rule, not of depth.
  dynamics    : bank programs vs ORACLE (true) dynamics at depth 2 --
                if oracle depth 2 helps where bank depth 2 does not,
                one-step model error compounds under composition.
  selection   : goals selected at depth 1 vs selected at depth 2
                (leaf) -- a goal only useful under lookahead is never
                chosen by a depth-1 selector (co-adaptation).

Cells per world (all evaluated on the untouched stream seed*977):
  sel-d1: exec-d1-bank, exec-d2leaf-bank, exec-d2leaf-oracle,
          exec-d1-oracle
  sel-d2leaf: exec-d2leaf-bank (complete depth-2 system),
              exec-d1-bank (co-adaptation control)

Readings (registered):
  - d2leaf recovers what d2cum lost        -> aggregation artifact.
  - oracle-d2 > oracle-d1 while bank-d2 <= bank-d1 -> model error
    compounds; the planner needs a better model, not deeper search.
  - sel-d2/exec-d2 > sel-d1/exec-d2        -> horizon must co-scale
    with selection (the co-scaling law again, now for depth).
  - no cell improves on exec-d1            -> this family at this
    horizon does not reward lookahead. Scope: fixed on this family.
Statistics are reported per-seed and per-world (clustered), not only
pooled.
"""
    ...
    for _ in range(steps):
        ...
        if mode == "random":
            ...
            for act in range(4):
                ...
                if depth > 1 and mode == "bank":
                    deeper = None
                    for nxt in range(4):
                        program = bank.get(nxt)
                        after = (state if program is None
                                 else executor(program, state))
                        c2 = goal_cost(after, reference, goal)
...
def choose_composite(config, bank, allowed, depth=1):
...
for name, config in WORLDS:
    ...
    row = {}
    for sel_arm, sel_depth in (("sel_d1", 1), ("sel_d2", 2)):
        best = None
        for label, candidate in ENC_CANDIDATES.items():
            globals()["enc"] = candidate
            goal = choose_composite(config, banks[label],
                                    frozenset(range(SLOTS)),
                                    depth=sel_depth)
            if goal is None:
                continue
        ...
        cell = {"encoder": label,
                ...
                "exec_d1": play(config, "bank", bank, args.seed * 977,
                                goal, plant_executor, args.episodes,
                                args.steps),
                "exec_d2leaf": play(config, "bank", bank, args.seed * 977,
                                    goal, plant_executor, args.episodes,
                                    args.steps, depth=2)}
        if sel_arm == "sel_d1":
            cell["oracle_d1"] = play(config, "oracle", None,
                                     args.seed * 977, goal,
                                     plant_executor, args.episodes,
                                     args.steps)
            cell["oracle_d2leaf"] = play(config, "oracle", None,
                                         args.seed * 977, goal,
                                         plant_executor, args.episodes,
                                         args.steps, depth=2)
        row[sel_arm] = cell
    ...
    results[name] = row
    a, b = row.get("sel_d1", {}), row.get("sel_d2", {})
    print(f"  {name:<16} rnd {row['random']:+.3f}  "
          f"d1 {a.get('exec_d1', float('nan')):+.3f}  "
          f"d2leaf {a.get('exec_d2leaf', float('nan')):+.3f}  "
          f"orc_d1 {a.get('oracle_d1', float('nan')):+.3f}  "
          f"orc_d2 {a.get('oracle_d2leaf', float('nan')):+.3f}  "
          f"sel2/d2 {b.get('exec_d2leaf', float('nan')):+.3f}  "
          f"sel2/d1 {b.get('exec_d1', float('nan')):+.3f}", flush=True)
