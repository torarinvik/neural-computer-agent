"""GUARDED TRANSFER: imports must beat a local baseline (F243).

F242 named two transfer failure modes: an imported form scoring BELOW
random on gcollect2 (transfer can mislead), and one seed's library
abstracting to nothing. The guard specified there, implemented: score
the re-grounded imports on the selection streams ALONGSIDE the random
policy; adopt the best import only if it clears random by a margin,
otherwise FALL BACK to the full local race -- discovery cost is paid
only where transfer fails.

Registered predictions:
  1. gcollect2: the guard trips in >= 5/6 seeds and the fallback
     restores ~cold performance -- the below-random harm is gone.
  2. gcollect1 / gcollect1_avoid1: guard passes, imports adopted,
     ~700x savings retained.
  3. Pooled guarded-warm >= 98% of cold at a fraction of its cost;
     the empty-library seed routes to fallback by construction.
"""
...
def choose_goal_local(config, bank):
...
for name, config in WORLDS:
    ...
    spent += 2 * args.search_episodes
    source = "import"
    if best is None or best_score <= baseline + GUARD_MARGIN:
        source = "fallback"
        best, best_score, race_spent = choose_goal_local(config, bank)
        spent += race_spent
    if best is None:
        continue
    row = {"goal": str(best), "source": source,
           "guard_baseline": round(baseline, 4),
           "import_score": (round(best_score, 4) if source == "import"
                            else None),
    ...
    print(f"  {name:<18} random {row['random']:+.3f}  warm "
          f"{row['bank']:+.3f} ({spent}ep, {source})  goal "
          f"{row['goal']}", flush=True)
