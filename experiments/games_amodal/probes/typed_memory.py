"""TYPE-KEYED TRANSITION MEMORY: signature-pure dynamics entries
(F259).

F258 died the third death by group-slot mixture: transition entries
pooled across rows where the same slot-group tracks different
entity types. The typed reward head survived that exact hazard by
conditioning on the entity signature psi; this probe gives dynamics
the same conditioning. The table key gains a SIGNATURE BUCKET
(mover? approacher? -- 4 buckets from the tracker's motion energy
and approach fraction), making every entry type-pure:

  key = (bucket, entity cell, post-action avatar cell)
  fallback = (bucket, entity cell) marginal; FROZEN below evidence.

Everything else is F258 unchanged (avatar via plant-executed bank
program, F256 transferred head refit table-consistently on the six
simple worlds, trio + control targets).

Registered predictions (before any run):
  P1 the mixture was the defect: memtr_d4 >= transfer_d4 + 0.10 on
     both single-interceptor trio worlds (the F257 prize collected).
  P2 delayed3_i1_p1_r2: memtr_d4 >= -0.40 (approaching the
     truedyn+transfer ceiling -0.29).
  P3 double-interceptor world: memtr_d4 >= transfer_d4 - 0.05 (no
     harm; logged as found).
        ...
        for g, (_pl, base) in enumerate(TRACKED):
            ...
            for i in valid.nonzero().flatten().tolist():
                ...
                full[(kf * GRID + int(ar[i])) * GRID + int(ac[i])][
                    (dr, dc)] += 1
                marg[kf][(dr, dc)] += 1
    TF = torch.zeros(4 * 3 * GRID ** 4, 2, dtype=torch.long)
    VF = torch.zeros(4 * 3 * GRID ** 4, dtype=torch.bool)
    for k, c in enumerate(full):
        if c:
            d = c.most_common(1)[0][0]
            TF[k, 0], TF[k, 1] = d
            VF[k] = True
    TM = torch.zeros(4 * 3 * GRID ** 2, 2, dtype=torch.long)
    VM = torch.zeros(4 * 3 * GRID ** 2, dtype=torch.bool)
...
def table_roll(parent, av_r, av_c, table, buckets):
    """Entities via the type-keyed transition table (full key,
    marginal fallback, FROZEN default); buckets [N, 3]."""
    TF, VF, TM, VM = table
    child = parent.clone()
...
def plan_mem(reference, psi, bank, table, w_r, w_v, depth):
    ...
    for level in range(depth):
        ...
        flat = S.reshape(-1, SLOTS)
        psi_t = psi.repeat(K, 1, 1)
        buckets_t = buckets_root.repeat(K, 1)
        for act in range(4):
            program = bank.get(act)
