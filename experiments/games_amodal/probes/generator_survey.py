"""GENERATOR SURVEY: the library meets randomly sampled worlds (F245).

Twenty GEN-DEV worlds sampled from the mechanism space (the sealed
half's manifest was committed before this probe existed). Each world
gets the guarded-transfer protocol: the canonical promoted library is
scored against the random-policy baseline; if no import clears it,
the batched fast-stack singles race runs as fallback. Worlds where
even the fallback barely beats random are flagged as WITNESS
CANDIDATES -- found by sampling, not manufactured.

Registered predictions:
  1. Imports adopted on >= 50% of generated worlds.
  2. Guarded performance >= random everywhere (the guard's safety
     promise generalizes to sampled mechanism space).
  3. Flagged witness candidates (bank - random < +0.2) exist and are
     concentrated in mechanism combinations the DEV originals never
     exhibited -- the generator's first real produce.
"""
...
_dev = _json.load(open(
    "experiments/games_amodal/sealed/GEN_DEV_v1.json"))["configs"]
from experiments.games_amodal.world_generator import name_of
WORLDS = [(name_of(c), FamilyConfig(**c)) for c in _dev]
...
LIBRARY = [
    ...
    ("sched", ("schedule", [(((0, 1), (4, 5), 1),),
                            (((0, 1), (2, 3), 1),)])),
]


def score2(config, bank, goal):
    return min(play(config, "bank", bank, args.seed * 977 + off, goal,
...
for name, config in WORLDS:
    ...
    for label, goal in LIBRARY:
        s = score2(config, bank, goal)
        if best_score is None or s > best_score:
            best, best_score, best_label = goal, s, label
    baseline = min(play(config, "random", None, args.seed * 977 + off,
                        None, None, args.search_episodes,
                        args.search_steps) for off in (1, 2))
    source = "import"
    if best_score <= baseline:
        source = "fallback"
        picked = choose_by_race(config, bank, "second2")
        if picked is not None:
            best, best_score = picked
            best_label = "raced"
    rnd = play(config, "random", None, args.seed * 977, None, None,
               args.episodes, args.steps)
    bankscore = play(config, "bank", bank, args.seed * 977, best,
                     plant_executor, args.episodes, args.steps)
    witness = bool(bankscore - rnd < 0.2)
    adopted += source == "import"
    flagged += witness
    results[name] = {"source": source, "form": best_label,
                     "random": rnd, "bank": bankscore,
                     "witness_candidate": witness}
    mark = " <== WITNESS?" if witness else ""
    print(f"  {name:<44} rnd {rnd:+.3f}  bank {bankscore:+.3f} "
          f"({source}/{best_label}){mark}", flush=True)

results["_adopted"] = adopted
results["_flagged"] = flagged
print(f"  adopted {adopted}/{len(WORLDS)}  witness candidates "
