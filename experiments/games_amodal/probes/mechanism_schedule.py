"""GOAL SCHEDULES: sequencing inside the frozen grammar.

F230's hard witness: resource1 (+0.11 vs ~+1.5) -- a SUM of distance
terms cannot say "resource first, then food". The minimal machinery
that can: a SCHEDULE, an ordered pair of ordinary pair-goals that
advances cyclically when the active goal is COMPLETED (its cost
reaches zero at the current reference: you have arrived). No new
perception, no new ISA, no new goal terms -- composition in TIME.

Selection: score all admissible singles with the robust protocol
(unchanged), take the top 4, race every ordered pair of distinct
singles as a cyclic 2-phase schedule, and keep whatever wins --
schedule or plain single -- by the same min-of-two-streams rule.

Registered predictions:
  1. resource1 recovers to >= +0.8 with the schedule (approach
     resource -> approach food -> repeat); the winning schedule's two
     phases name plane 2 then plane 1 in a majority of seeds.
  2. Controls do not regress: collect2, delayed3, avoid1_collect1
     keep their F230 numbers (the plain single remains available and
     should win where one goal suffices).
  3. deceptive1: schedules do not fix the bait tax (it is a value
     problem, not a sequencing problem) -- expected unchanged.
"""
...
def choose_composite(config, bank, allowed):
    ...
    for i in range(len(top)):
        for j in range(len(top)):
            if i != j:
                candidate = ("schedule", [(top[i],), (top[j],)])
                r = score(candidate)
                if r > best_r:
                    best_c, best_r = candidate, r
    return best_c
...
for name, config in WORLDS:
    ...
    print(f"  {name:<16} random {row['random']:+.3f}  bank "
          f"{row['bank']:+.3f}  [{label}] goal {row['goal']}", flush=True)
