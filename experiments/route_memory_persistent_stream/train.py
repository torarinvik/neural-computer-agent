...
def _initial_state() -> tuple[
    ...
    redundant = [basis[2], _normalize(0.98 * basis[2] + 0.2 * basis[3])]
...
def _stream(seed: int, *, learn: bool) -> dict[str, object]:
    ...
    for step in range(STREAM_STEPS):
        ...
        utility = 1.0 if committed_now else 0.0
        if learn:
...
def run(seed: int, report_out: Path) -> dict[str, object]:
    ...
    report = {
        "schema": "neural-computer.route-memory-persistent-stream.v1",
        "claim_boundary": (
            "one persistent route-memory stream with verifier-gated online "
            "maintenance, interference, reversal, and growth cost; not universal "
