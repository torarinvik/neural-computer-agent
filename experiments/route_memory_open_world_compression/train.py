...
def _initial_state() -> tuple[
    ...
    for route in (*anchors[1:], distractor):
        memory.observe(SLOT_ID, route)
    return memory, [*anchors], [*anchors, distractor]
...
def _event(step: int) -> str:
    return ("new", "duplicate_pressure", "duplicate_admit", "compress")[
        step % 4
    ]


def _latent(seed: int, cycle: int, reversal: bool) -> torch.Tensor:
...
def _query(seed: int, step: int, reversal: bool) -> torch.Tensor:
    latent = _latent(seed, step // 4, reversal)
    if step % 4 in {1, 2}:
        generator = torch.Generator().manual_seed(seed + 50000 + step // 4)
        latent = _normalize(
            latent + 0.1 * torch.randn(WIDTH, generator=generator)
        )
    return latent
...
def _target(
    ...
    if event in {"duplicate_pressure", "duplicate_admit"}:
        if count == capacity:
...
def _stream(seed: int, *, learn: bool) -> dict[str, object]:
    ...
    completed_cycles = 0
    attempts = 0
    for cycle in range(LATENT_CYCLES):
        reversal = cycle >= REVERSAL_STEP
        latent = _latent(seed, cycle, reversal)
        duplicate = _query(seed, cycle * 4 + 1, reversal)
        stage = "new"
        cycle_complete = False
        while attempts < STREAM_STEPS and not cycle_complete:
            attempts += 1
            query = latent if stage == "new" else duplicate
            event = stage
            ...
            if not isinstance(plan, CapacityPlan):
                raise TypeError("open-world stream planner returned multiple plans")
            ...
            if committed_now:
                if stage == "new" and target_action in {"admit", "evict"}:
                    stage = "duplicate_pressure"
                elif stage == "duplicate_pressure":
                    stage = "duplicate_admit" if target_action == "grow" else "compress"
                elif stage == "duplicate_admit" and target_action in {"admit", "evict"}:
                    stage = "compress"
                elif stage == "compress" and target_action == "consolidate":
                    mastered.append(latent)
                    cycle_complete = True
                    completed_cycles += 1
            retained = all(
                memory.propose(
...
def run(seed: int, report_out: Path) -> dict[str, object]:
    ...
    gates = {
        ...
        "compression_is_observed": trained["compression_events"] >= LATENT_CYCLES // 2,
        "unseeded_discovery": trained["compression_events"] > 0,
        "reversal_stream_completed": trained["completed_cycles"] == LATENT_CYCLES,
        "controller_frozen": controller_digest == _digest(controller),
        "replay_zero": True,
        "one_update_per_unique_utility": True,
    }
    report = {
        "schema": "neural-computer.route-memory-open-world-compression.v1",
        "claim_boundary": (
            "one persistent route-memory stream that discovers and compresses "
            "unseeded redundant evidence under verifier-gated maintenance, "
            "reversal, and growth cost; not universal continual learning or "
        ...
        "accounting": {
            "unique_verifier_utilities": len(trained["utilities"]),
            "unique_logical_lifetimes": LATENT_CYCLES,
            "optimizer_updates": len(trained["utilities"]),
            "replayed_examples": 0,
