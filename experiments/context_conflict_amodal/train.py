"""Train and audit outcome-only contextual contradiction arbitration."""
...
from .environment import BalancedContextConflictCurriculum, ContextConflictVerifier


class FrozenEventEncoder(nn.Module):
    """Independent frozen frontend producing standardized learned events."""

    def __init__(self, raw_width: int, event_width: int, source_key: list[float]) -> None:
        super().__init__()
        self.network = nn.Sequential(
            nn.Linear(raw_width, event_width), nn.Tanh(),
            nn.Linear(event_width, event_width), nn.Tanh(),
        )
...
def build_runtime(*, seed: int = 0, width: int = 32) -> AmodalControllerRuntime:
    ...
    encoders = {
        name: FrozenEventEncoder(
            ContextConflictVerifier.raw_width,
            width,
            [1.0 if index == position else 0.0 for position in range(3)],
        )
        for index, name in enumerate(ContextConflictVerifier.stream_names)
    }
...
def train_steps(
    runtime: AmodalControllerRuntime,
    verifier: ContextConflictVerifier,
    *,
    steps: int,
    batch_size: int,
    seed: int,
    ...
    for step in range(1, steps + 1):
        ...
        if step == 1 or step % eval_every == 0 or step == steps:
            score = evaluate_condition(
                runtime,
                ContextConflictVerifier(seed=seed + 1000 + step, device=device),
                condition="clean",
                batches=4,
    ...
    for index, point in enumerate(history):
        if point["clean_reward"] >= threshold and all(
            ...
            stable_bits = int(point["step"] * batch_size * verifier.bit_count)
            break
...
def _condition_verifier(
    ...
    if condition == "context_zero":
        return ContextConflictVerifier(seed=seed, device=device, force_context=0)
    if condition == "context_one":
        return ContextConflictVerifier(seed=seed, device=device, force_context=1)
    if condition == "assignment_shuffled":
        return ContextConflictVerifier(seed=seed, device=device, shuffle_assignment=True)
    if condition == "context_inverted":
        return ContextConflictVerifier(seed=seed, device=device, invert_context=True)
    if condition == "stream_order_shuffled":
        return ContextConflictVerifier(seed=seed, device=device, stream_order_shuffle=True)
    return ContextConflictVerifier(seed=seed, device=device)
...
def run_experiment(
    *,
    ...
    history, accounting = train_steps(
        runtime,
        BalancedContextConflictCurriculum(seed=seed + 10, device=device),
        steps=steps,
        batch_size=batch_size,
        seed=seed,
    ...
    promotion = (
        ...
        and conditions["context_zero"] >= 0.80
        and conditions["context_one"] >= 0.80
        and conditions["stream_order_shuffled"] >= 0.80
        and conditions["assignment_shuffled"] <= 0.65
        and conditions["context_inverted"] <= 0.65
        and conditions["action_shuffled"] <= 0.65
        and conditions["intention_shuffled"] <= 0.65
        and conditions["intention_zero"] <= 0.65
    )
    report = {
        "experiment": "outcome-only-contextual-contradiction-arbitration",
        "seed": seed,
        "steps": steps,
        "batch_size": batch_size,
        ...
        "hidden_rule": "context 0 trusts candidate b; context 1 trusts candidate c",
        "history": history,
        "conditions": conditions,
        "promotion_gate": {
            "clean_and_both_contexts_min": 0.80,
            "stream_order_shuffled_min": 0.80,
            "assignment_shuffle_max": 0.65,
            "context_inversion_max": 0.65,
