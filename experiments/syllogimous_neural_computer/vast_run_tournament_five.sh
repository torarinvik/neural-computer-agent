...
BASE=experiments/syllogimous_neural_computer/targeted_tournament_pilot
OUT=experiments/syllogimous_neural_computer/targeted_tournament_five
mkdir -p "$OUT"
for seed in 11 23 37; do
  "$PYTHON" -m experiments.syllogimous_neural_computer.train_consolidation \
    ...
    --initial-policy "$BASE/seed_${seed}.pt" \
