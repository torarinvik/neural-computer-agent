"""Replay an independently saved latent-basis adapter through the runtime.

This audit intentionally does not import a training optimizer or a training
report. It reconstructs the frozen controller, input bus, original frontend,
and decoder from their promoted artifacts, loads only the saved adapter
weights, and reruns the composition and causal controls.
"""
...
from .train_amodal_latent_alignment import (
    LatentBasisFrontend,
    evaluate_alignment,
)
...
def _runtime_from_saved_adapter(
    ...
    if artifact.get("schema") != "amodal-latent-basis-adapter-v1":
        raise ValueError("unsupported latent adapter artifact schema")
    if artifact.get("controller_sha256") != _sha256(controller_path):
        raise ValueError("adapter was trained against a different controller")
    if artifact.get("input_bus_sha256") != _sha256(bus_path):
        raise ValueError("adapter was trained against a different input bus")

    extracted = runtime_from_legacy_payload(controller_payload, device=device).eval()
    event_width = int(bus_payload["event_width"])
    bus = AmodalInputBus(event_width, int(bus_payload["residual_hidden"])).to(device)
    ...
    permutation = artifact["basis_permutation"].to(device=device)
    if not torch.equal(permutation, adapter.basis_permutation):
        raise ValueError("adapter basis permutation does not match runtime")
    adapter.adapter.load_state_dict(
        {
            name: value.to(device=device)
            for name, value in artifact["adapter_state_dict"].items()
        }
    )
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=987_001)
    ...
    runtime = _runtime_from_saved_adapter(
    ...
    report = {
        "schema": "amodal-latent-basis-adapter-replay-v1",
