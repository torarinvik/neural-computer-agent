...
parser.add_argument(
    ...
    help="fraction of data-collection actions taken TOWARD the nearest "
         ...
         "learning from.")
parser.add_argument(
    "--seek-plane2", type=float, default=0.0,
    help="probability that a seek action targets the plane-2 object "
         "rather than plane-1. 0.0 reproduces F113/F118; 0.5 is the "
         "F123 collapse; intermediate values trade channel coverage "
         "against the twin asymmetry that makes reading pay.")
parser.add_argument(
...
parser.add_argument(
    "--oracle-outcome", action="store_true",
    help="ORACLE SUBSTITUTION: replace the outcome model with ground "
         "truth inside the search, keeping the learned dynamics and the "
         "beam intact. F109 eliminated three candidate explanations for "
         "the missing ~75%% of headroom and left the outcome model as "
         "the obvious suspect without showing it BINDING. This settles "
         "it in one run: if performance jumps toward the +0.1954 oracle "
         "the outcome model is the constraint; if it does not, the "
         "search or the horizon is. Truth is read from the verifier's "
         "item layout, which is legitimate for an ablation and never "
         "reaches the learner in any other arm.")
parser.add_argument(
    "--value-head", action="store_true",
    help="regress the discounted n-step RETURN of arriving at each cell "
         "instead of classifying 3 outcome bins. F110's oracle "
         "substitution measured the outcome model to be the binding "
         "constraint (+0.1439 of the gap): 3-class labels quantise all "
         "positive futures to one bin and were degenerate twice over "
         "(F102 all-nothing, F106 never-nothing). The oracle supplies "
         "REAL-VALUED cell worth, so the fix mimics its interface: a "
...
parser.add_argument(
    ...
    help="add a multiplicative signed pathway to the value head: "
         ...
         "the entry SUPPRESSES the flagged object but cannot PROMOTE "
         "the newly-edible one (top=food 0.219 normal vs 0.021 "
         "inverted, poison avoidance transferring to both) — through "
         "attention alone a sign flip requires re-mapping the whole "
         "value surface. Here the state supplies a polarity-free object "
         "salience and the entry supplies one scalar in [-1, 1], so "
         "attraction and repulsion are a single sign apart by "
         "construction.")
...
def seek_actions(states: torch.Tensor, generator) -> torch.Tensor:
    """Move toward a nearby object, from the slot state the learner
    itself reads. No privileged information is used.

    `--seek-plane2 p` sets how often the target is the plane-2 object
    instead of plane-1. F118 measured that p=0 starves the plane-2
    salience channel (inverted worlds can only avoid, never seek), but
    F123 measured that p=0.5 destroys reading outright: balanced
    seeking makes the two twins' outcome marginals identical, the
    twin-average becomes a good fit, and the model settles there —
    F106's collapse re-created by a data change. The asymmetry was
    doing double duty. Intermediate p keeps enough asymmetry for
    reading to pay while still visiting plane-2.
    """
    ...
    for row in range(states.shape[0]):
        ...
        if int(states[row, base]) == ABSENT or \
            ...
            continue
        dr = int(states[row, base]) - int(states[row, 0])
        dc = int(states[row, base + 1]) - int(states[row, 1])
...
def roll(verifier: FamilyVerifier, count: int, generator) -> dict:
    ...
    return {"state": flat(states), "action": flat(acts),
            "next": flat(nxt), "outcome": labels, "value": value}
...
if not args.random_plant:
    ...
    for step in range(args.train_updates):
        ...
        if args.value_head:
            predicted_value = outcome(batch["state"], batch["action"],
                                      entry, value=True,
                                      signed=args.signed_entry)
            loss = loss + torch.nn.functional.smooth_l1_loss(
                predicted_value, batch["value"])
            if args.ignorance > 0:
                blind_value = outcome(batch["state"], batch["action"],
                                      torch.zeros_like(entry), value=True)
                # ignorant prediction must carry no information: pin it
                # to the batch mean rather than to the per-sample truth
                loss = loss + args.ignorance * torch.nn.functional.mse_loss(
                    blind_value,
                    batch["value"].mean().expand_as(blind_value))
        weight = None
...
def act(states: torch.Tensor, entry, truth=None) -> torch.Tensor:
    ...
    with torch.no_grad():
        for depth in range(args.depth):
            ...
            for action in range(ACTIONS):
                ...
                if truth is None:
                    if args.value_head:
                        gain = outcome(flat, column, entry,
                                       value=True,
                                       signed=args.signed_entry
                                       ).view(batch, width)
                    else:
                        probability = outcome(flat, column,
                                              entry).softmax(-1)
                        gain = (probability[:, 2]
                                - probability[:, 0]).view(batch, width)
                else:
                    # true value of the cell the avatar would land on
                    cells = (successor[:, :, 0].clamp(0, HEIGHT - 1) * WIDTH
                             + successor[:, :, 1].clamp(0, WIDTH - 1))
                    gain = torch.gather(truth, 1, cells)
...
def truth_map(verifier: FamilyVerifier) -> torch.Tensor:
    """+1 on cells holding an edible item, -1 on costly ones, else 0.

    `inverted` swaps which plane is food, which is exactly the bit the
    bank entry is supposed to carry — so this map is the information the
    outcome model is failing to supply.
    """
    out = torch.zeros(verifier.batch_size, HEIGHT * WIDTH)
    good, bad = (verifier._forage_b, verifier._forage_a) \
        if verifier.config.inverted else (verifier._forage_a,
                                          verifier._forage_b)
    for row in range(verifier.batch_size):
        for cell in good[row]:
            out[row, cell[0] * WIDTH + cell[1]] = 1.0
        for cell in bad[row]:
            out[row, cell[0] * WIDTH + cell[1]] = -1.0
    return out
...
def play(config: FamilyConfig, entry, seed: int,
    ...
    for _ in range(args.trials):
        ...
        choice = (torch.randint(0, ACTIONS, (verifier.batch_size,),
                                generator=generator)
                  if random_actions else
                  act(state, entry,
                      truth_map(verifier) if args.oracle_outcome else None))
...
def value_fidelity(config: FamilyConfig) -> dict:
    """Predicted cell worth vs GROUND TRUTH cell worth, directly.

    F111 closed part of the gap with a value head; the remainder to the
    +0.1234 oracle target is either value inaccuracy or the search. This
    compares the value head's per-cell prediction against the verifier's
    true layout: correlation, and — because the search only takes
    argmaxes — whether the TOP-RANKED cell is truly food, truly poison,
    or empty.
    """
    verifier = verifier_for(config, args.seed + 777)
    entry = reader(roll(verifier_for(config, args.seed + 31),
                        args.context, probe))
    state = slot_state(verifier.observation().reshape(
        verifier.batch_size, -1))
    truth = truth_map(verifier)
    correlations, top_food, top_poison = [], 0, 0
    with torch.no_grad():
        for row in range(min(verifier.batch_size, 16)):
            cells = torch.arange(HEIGHT * WIDTH)
            rows_state = state[row].unsqueeze(0).expand(
                HEIGHT * WIDTH, SLOTS).clone()
            rows_state[:, 0] = cells // WIDTH
            rows_state[:, 1] = cells % WIDTH
            acts = torch.zeros(HEIGHT * WIDTH, dtype=torch.long)
            predicted = outcome(rows_state, acts, entry, value=True,
                                signed=args.signed_entry) \
                if args.value_head else \
                (outcome(rows_state, acts, entry).softmax(-1)[:, 2]
                 - outcome(rows_state, acts, entry).softmax(-1)[:, 0])
            t = truth[row]
            if float(t.abs().sum()) == 0:
                continue
            pc = predicted - predicted.mean()
            tc = t - t.mean()
            denom = float(pc.norm() * tc.norm())
            if denom > 0:
                correlations.append(float((pc @ tc) / denom))
            best = int(predicted.argmax())
...
report = {"seed": args.seed, "train_count": len(train),
          ...
          "value_fidelity": {c.name: value_fidelity(c) for c in held[:6]}}
