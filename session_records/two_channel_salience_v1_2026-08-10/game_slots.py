...
def seek_actions(states: torch.Tensor, generator) -> torch.Tensor:
    ...
    itself reads. No privileged information is used. F118: target the
    plane-1 and plane-2 object with equal probability — seeking only
    plane-1 leaves plane-2 consumption events almost absent from the
    data, so inverted worlds (where plane-2 is the food) have nothing
    to learn "seek" from and can only ever learn avoidance."""
    action = torch.randint(0, ACTIONS, (states.shape[0],),
                           generator=generator)
    plane = torch.randint(0, 2, (states.shape[0],), generator=generator)
