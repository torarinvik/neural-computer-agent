...
for name, config in WORLDS:
    ...
    for label, candidate in ENC_CANDIDATES.items():
        ...
        if goal is None:
            continue
        if args.composite:
            # F222's greedy composition, here where it can finally name
            # BOTH trackers: singles were already scored by choose_goal's
            # internals, so re-score the top and try pairs
            usable = usable_slots(config)
            singles = []
            for pa in pairs:
                for pb in pairs:
            ...
            def score(g):
                return play(config, "bank", bank, args.seed * 977 + 1, g,
                            plant_executor, args.search_episodes,
                            args.search_steps)
            scored = sorted(((score((t,)), t) for t in singles),
                            key=lambda x: -x[0])
    ...
    row = {"encoder": label,
           "goal": ([[list(t[0]), list(t[1]), t[2]] for t in goal]
                    if not isinstance(goal[0][0], int)
                    else [list(goal[0]), list(goal[1]), goal[2]]),
