...
class GoalReader(torch.nn.Module):
    ...
    def forward(self, before, after, reward):
        ...
        # Iteration 4 normalised these by event mass to make the sign
        # visible; the pairs destabilised and the net effect was worse
        # (F217's table). Reverted: the reader's job is the PAIR, which
        # dynamics carry; the sign is resolved downstream by two rollout
        # evaluations, which is where one bit of reward information is
        # actually cheap to get.
        feature = torch.cat([
...
for config in held_grids:
    ...
    # READ + VERIFY, F208's pattern for goals: the reader supplies the
    # pair (readable from dynamics), and the one unreadable bit -- the
    # sign -- is settled by evaluating BOTH signs with two cheap
    # rollouts. 360 rollout evaluations become 2.
    verified = None
    best_r = -1e9
    for sign in (1, -1):
        candidate = (read[0], read[1], sign)
        r = play(config, "bank", bank, args.seed * 977 + 1, candidate,
                 args.search_episodes, args.search_steps)
        if r > best_r:
            verified, best_r = candidate, r
    # BEAM of two pairs: the reader's pair plus the wake pool's mode
    # pair, each at both signs -- four rollout evaluations against the
    # search's 360. The reader's pair carries the world-specific reading
    # (it is what wins on avoid and navigate); the mode pair is the
    # accumulated prior (it is what wins where the reader misreads).
    # F208 for goals: read what is readable, verify a shortlist.
    beam = None
    beam_r = -1e9
    for pair in {(read[0], read[1]), (mode_goal[0], mode_goal[1])}:
        for sign in (1, -1):
            candidate = (pair[0], pair[1], sign)
            r = play(config, "bank", bank, args.seed * 977 + 1, candidate,
                     args.search_episodes, args.search_steps)
            if r > beam_r:
                beam, beam_r = candidate, r
    row = {"searched_goal": [list(searched[0]), list(searched[1]),
                             searched[2]],
           "verified_goal": [list(verified[0]), list(verified[1]),
                             verified[2]],
           "beam_goal": [list(beam[0]), list(beam[1]), beam[2]],
...
for label in ("random", "searched", "read", "read_verified",
              "read_beam", "read_scrambled", "mode"):
