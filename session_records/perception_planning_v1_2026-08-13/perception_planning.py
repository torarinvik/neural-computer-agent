...
def _nearest(plane, ref_row, ref_col, which):
    """Generic relational reduction: the position, within `plane`, of the
    set cell minimising L1 distance to a reference coordinate pair. The
    reference is ANOTHER FEATURE's output -- no channel is special, no
    avatar is mentioned; which reference matters is left to selection."""
    mask = plane > 0
    d = ((ROWS_IX.unsqueeze(0) - ref_row.view(-1, 1, 1)).abs()
         + (COLS_IX.unsqueeze(0) - ref_col.view(-1, 1, 1)).abs())
    d = torch.where(mask, d, torch.full_like(d, 999))
    flat = d.reshape(d.shape[0], -1).argmin(dim=1)
    return flat // WIDTH if which == "row" else flat % WIDTH


def relational_encoder(anchor_channel, anchor_kind="peak"):
    """peaks-of-anchor plus nearest-to-anchor for the other channels.
    With anchor_channel=0 this SUBSUMES the hand-written slot_state as
    one member of a generated family; anchors 1 and 2 are its equally
    expressible siblings that no human would have written."""
    def encoder(screen):
        frames = screen.view(-1, PLANES, HEIGHT, WIDTH)
        ar = _reduce(frames[:, anchor_channel], f"{anchor_kind}_row")
        ac = _reduce(frames[:, anchor_channel], f"{anchor_kind}_col")
        out = torch.zeros((frames.shape[0], SLOTS), dtype=torch.long)
        out[:, 0], out[:, 1] = ar.clamp(0, VALUES - 1), ac.clamp(0, VALUES - 1)
        others = [c for c in range(PLANES) if c != anchor_channel]
        for i, c in enumerate(others):
            out[:, 2 + 2 * i] = _nearest(frames[:, c], ar, ac,
                                         "row").clamp(0, VALUES - 1)
            out[:, 3 + 2 * i] = _nearest(frames[:, c], ar, ac,
                                         "col").clamp(0, VALUES - 1)
        for i, c in enumerate([anchor_channel] + others):
            empty = ~(frames[:, c] > 0).any(dim=(1, 2))
            out[empty, 2 * i] = ABSENT
            out[empty, 2 * i + 1] = ABSENT
        return out
    return encoder


CANDIDATES = {
    "rel_anchor0": relational_encoder(0),
    "rel_anchor1": relational_encoder(1),
    "rel_anchor2": relational_encoder(2),
    "rel_centre0": relational_encoder(0, "centre"),
