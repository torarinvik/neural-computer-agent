...
parser.add_argument(
    ...
    help="sample slot COUNTS uniformly instead of uniformly over "
         "feasible (slots, values) pairs. F88: read accuracy falls "
         "monotonically with slot count (0.996 at 1 slot, 0.870 at 6) "
         "and rejection sampling leaves 6-slot families at 3.7% of the "
         "pool. F89: giving the entry more capacity made those families "
         "WORSE, so the fix is representation, not width.")
parser.add_argument(
    "--gated", action="store_true",
    help="add POSITION-DEPENDENT op primitives (a barrier that refuses "
         "a move onto one value, and an effect conditional on another "
         "slot). F92: the reacher's walled grid is the mechanism's only "
         "decisive failure (438 updates vs a cold 88) because every "
         "other op is a uniform function of slot VALUES. This is the "
         "direct analogue of the fix that took `toggle` from 0.096 to "
         "0.917 -- supply the missing primitive rather than more "
         "capacity or more training.")
parser.add_argument("--store-key", choices=("mean", "concat"),
                    default="concat",
                    help="how an exception is addressed. `mean` pools slot "
                         "embeddings and collides; `concat` keeps slot "
                         "identity.")
...
def build_pool() -> list:
    ...
    for index in range(args.pool):
        family = RandomFamily(
            random_family_spec(generator, wide=args.wide,
                               balanced=args.balanced,
                               gated=args.gated), f"proc{index}")
        if args.scramble:
            size = len(family.states)
...
# ---- EXCEPTION STORE (F96) ----
# The bank entry names a RULE. `walled` reads 0.894 because the rule is
# "8x8 grid movement" and the 27 blocked transitions are not a rule but
# a LIST -- ~121 bits of arbitrary content no entry can carry. The fix
# is a store, not a better rule: watch the world, record where the rule
# is WRONG, and consult that first. Nothing is trained; the plant stays
# frozen and the entry is unchanged.
#
# Exceptions come from the SAME observed transitions that produced the
# entry, so this buys nothing the reader was not already shown. Its
# coverage is therefore limited by how much has been watched, which is
# the honest axis to report: 128 random draws from 256 possible pairs
# cover only ~39% of them.

def build_exceptions(plant, family: Family, entry, count: int,
                     generator) -> dict:
    """Record (state, action) -> outcome wherever the rule mispredicts."""
    states, acts, nxt = sample(family, count, generator)
    with torch.no_grad():
        logits = plant(family.slot_values(states), acts, entry)
    blocks = logits.argmax(-1)
    want = family.slot_targets(nxt)
    ...
    for row in range(states.shape[0]):
        wrong = any(int(blocks[row, slot]) != int(want[row, slot])
                    for slot in range(family.slots))
        if wrong:
            store[(int(states[row]), int(acts[row]))] = int(nxt[row])
    return store


def accuracy_with_exceptions(plant, family: Family, entry,
                             store: dict) -> float:
    """Exhaustive accuracy with the exception store consulted first."""
    size = len(family.states)
    states = torch.arange(size).repeat_interleave(family.actions)
    acts = torch.arange(family.actions).repeat(size)
    with torch.no_grad():
        logits = plant(family.slot_values(states), acts, entry)
    blocks = logits.argmax(-1)
    truth = [family.table[int(a)][int(b)] for a, b in zip(states, acts)]
    want = family.slot_targets(torch.tensor(truth))
    hits = 0
    for row in range(states.shape[0]):
        key = (int(states[row]), int(acts[row]))
        if key in store:
            hits += int(store[key] == truth[row])
            continue
        hits += int(all(int(blocks[row, slot]) == int(want[row, slot])
                        for slot in range(family.slots)))
    return round(hits / states.shape[0], 4)


def store_key(family: Family, states: torch.Tensor,
              acts: torch.Tensor) -> torch.Tensor:
    """Content address for an exception, from the PLANT's own frozen
    embeddings -- not from the integer index. F97's store was an exact
    dictionary keyed by (state, action), which is an idealised memory:
    it never mis-fires and never runs out of room. A real
    content-addressed store retrieves by similarity and has a capacity,
    so it can both MISS a stored exception and FIRE on the wrong one.
    Those two failure modes are what this measures."""
    with torch.no_grad():
        values = plant.value_embed(family.slot_values(states))
        if args.store_key == "mean":
            key = values.mean(1) + plant.action_embed(acts)
        else:
            # CONCATENATED slot embeddings. Mean-pooling over slots is
            # lossy -- two different states can share a mean -- so keys
            # collide and the store both misses and mis-fires. Keeping
            # the slots separate preserves state identity, which is
            # exactly what an exception is indexed by.
            key = torch.cat([values.reshape(values.shape[0], -1),
                             plant.action_embed(acts)], dim=-1)
    return torch.nn.functional.normalize(key, dim=-1)


def build_store_approx(plant, family: Family, entry, count: int,
                       generator, capacity: int) -> tuple:
    """Capacity-bounded, similarity-addressed exception store."""
    states, acts, nxt = sample(family, count, generator)
    with torch.no_grad():
        logits = plant(family.slot_values(states), acts, entry)
    blocks = logits.argmax(-1)
    want = family.slot_targets(nxt)
    keys, outcomes, seen = [], [], 0
    all_keys = store_key(family, states, acts)
    for row in range(states.shape[0]):
        if all(int(blocks[row, slot]) == int(want[row, slot])
               for slot in range(family.slots)):
            continue
        seen += 1
        keys.append(all_keys[row])
        outcomes.append(int(nxt[row]))
        if capacity and len(keys) > capacity:
            keys.pop(0)
            outcomes.pop(0)
    return (torch.stack(keys) if keys else None), outcomes, seen


def accuracy_approx(plant, family: Family, entry, keys, outcomes,
    ...
    want = family.slot_targets(torch.tensor(truth))
    hits = 0
    similarity = (store_key(family, states, acts) @ keys.T
                  if keys is not None else None)
    for row in range(states.shape[0]):
        if similarity is not None:
            best = int(similarity[row].argmax())
            if float(similarity[row, best]) >= threshold:
                hits += int(outcomes[best] == truth[row])
                continue
        hits += int(all(int(blocks[row, slot]) == int(want[row, slot])
                        for slot in range(family.slots)))
    return round(hits / states.shape[0], 4)
...
exception_generator = torch.Generator().manual_seed(args.seed + 31337)
report["exceptions"] = {}
for name, family in held.items():
    row = {"rule_only": report["held_out"][name]["read_accuracy"]}
    for watched in (args.context, 2 * args.context, 4 * args.context,
                    8 * args.context):
        store = build_exceptions(plant, family, entries[name], watched,
                                 exception_generator)
        row[f"watch{watched}"] = {
            "accuracy": accuracy_with_exceptions(plant, family,
                                                 entries[name], store),
            "stored": len(store)}
    # approximate, capacity-bounded store: the realistic version
    for capacity in (0, 32):
        keys, outcomes, encountered = build_store_approx(
            plant, family, entries[name], 8 * args.context,
            exception_generator, capacity)
        row[f"approx_cap{capacity}"] = {
            "accuracy": accuracy_approx(plant, family, entries[name],
                                        keys, outcomes, 0.99),
            "stored": len(outcomes),
            "violations_seen": encountered}
    report["exceptions"][name] = row
...
names = list(held)
# F92: this null used each family's LIST NEIGHBOUR as the "wrong"
# context. That was sound while all four hand-made families were
# mutually unrelated, but `grid` and `walled` share 229 of 256
# transitions, so grid's null read 1.000 and looked like "the entry is
# decoration" when the withheld-bank control says otherwise (0.170).
# A wrong entry that is 90% right cannot falsify anything. Draw the
# wrong context from a FRESHLY GENERATED family instead, which is
# unrelated by construction and cannot be broken by adding
# near-duplicates to the held-out set later.
stranger_generator = torch.Generator().manual_seed(args.seed + 24601)
stranger_entries = [
    context_entry(encoder,
                  RandomFamily(random_family_spec(stranger_generator,
                                                  wide=args.wide,
                                                  balanced=args.balanced,
                                                  gated=args.gated)),
                  stranger_generator)
...
report["wrong_context"] = {
    name: round(sum(accuracy(plant, held[name], e)
                    for e in stranger_entries) / len(stranger_entries), 4)
    for name in names}
report["wrong_context_neighbour"] = {
