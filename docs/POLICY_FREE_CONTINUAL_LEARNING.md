...
goal fragment. The transfer/fresh challenger now receives explicit caller-owned
acquisition costs and records a v2 cost-aware receipt. Across seeds `80–103`,
`9/24` complete runs passed. Every
...
used zero replay and zero optimizer updates. Fifteen runs were correctly
rejected before the complete gate in the cost-aware rerun. The ledger is in
